...
<%
if Instr(session("AdminPurview"),"|302,")=0 then 
...
<BODY>
<%
dim Result,StartDate,EndDate,Keyword,Reachsum,unReachsum,Reachper,sqlstr,department,Depart,anywhere
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(unProductC1One) as a1,sum(ProductC1One) as b1,sum(unProductC2One) as a2,sum(ProductC2One) as b2,sum(unProductC3One) as a3,sum(ProductC3One) as b3,sum(unProductC4One) as a4,sum(ProductC4One) as b4 from manusys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
	...
	Reachper="一分厂"
    if b1=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，一分厂制程异常笔数0"
	else
    c1=a1/b1*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，一分厂制程异常笔数[<font color='red'>"&b1&"</font>]，制程超耗笔数[<font color='red'>"&a1&"</font>]，一分厂制程超耗率为：[<font color='red'>"&formatnumber(c1,2)&"%</font>]"
	end if
	end if
	if department="11" then
	Reachper="二分厂"
    if b2=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，二分厂制程异常笔数0"
	else
    c2=a2/b2*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，二分厂制程异常笔数[<font color='red'>"&b2&"</font>]，制程超耗笔数[<font color='red'>"&a2&"</font>]，二分厂制程超耗率为：[<font color='red'>"&formatnumber(c2,2)&"%</font>]"
	end if
	end if
	if department="12" then
	Reachper="三分厂"
    if b3=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，三分厂制程异常笔数0"
	else
    c3=a3/b3*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，三分厂制程异常笔数[<font color='red'>"&b3&"</font>]，制程超耗笔数[<font color='red'>"&a3&"</font>]，三分厂制程超耗率为：[<font color='red'>"&formatnumber(c3,2)&"%</font>]"
	end if
	end if
	if department="19" then
	Reachper="娄桥分厂"
    if b4=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，娄桥分厂制程异常笔数0"
	else
    c4=a4/b4*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，娄桥分厂制程异常笔数[<font color='red'>"&b4&"</font>]，制程超耗笔数[<font color='red'>"&a4&"</font>]，娄桥分厂制程超耗率为：[<font color='red'>"&formatnumber(c4,2)&"%</font>]"
	end if
	end if
  else
	sqlstr2="select top 1 unProductC1One as a1,ProductC1One as b1,ProductConsum1One as c1,unProductC2One as a2,ProductC2One as b2,ProductConsum2One as c2,unProductC3One as a3,ProductC3One as b3,ProductConsum3One as c3,unProductC4One as a4,ProductC4One as b4,ProductConsum4One as c4 from manusys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    a1=rs("a1")
    b1=rs("b1")
...
if Depart="10" then
anywhere = " and 分厂='一分厂'"
    Response.Write "统计时间：[<font color='red'>昨日</font>]，一分厂制程异常笔数[<font color='red'>"&b1&"</font>]，制程超耗笔数[<font color='red'>"&a1&"</font>]，一分厂制程超耗率为：[<font color='red'>"&formatnumber(c1,2)&"%</font>]"
elseif Depart="11" then
anywhere = " and 分厂='二分厂'"
    Response.Write "统计时间：[<font color='red'>昨日</font>]，二分厂制程异常笔数[<font color='red'>"&b2&"</font>]，制程超耗笔数[<font color='red'>"&a2&"</font>]，二分厂制程超耗率为：[<font color='red'>"&formatnumber(c2,2)&"%</font>]"
elseif Depart="12" then
anywhere = " and 分厂='三分厂'"
    Response.Write "统计时间：[<font color='red'>昨日</font>]，三分厂制程异常笔数[<font color='red'>"&b3&"</font>]，制程超耗笔数[<font color='red'>"&a3&"</font>]，三分厂制程超耗率为：[<font color='red'>"&formatnumber(c3,2)&"%</font>]"
elseif Depart="19" then
anywhere = " and 分厂='娄桥分厂'"
    Response.Write "统计时间：[<font color='red'>昨日</font>]，娄桥分厂制程异常笔数[<font color='red'>"&b4&"</font>]，制程超耗笔数[<font color='red'>"&a4&"</font>]，娄桥分厂制程超耗率为：[<font color='red'>"&formatnumber(c4,2)&"%</font>]"
else
anywhere = ""
    Response.Write "统计时间：[<font color='red'>昨日</font>]，一分厂制程异常笔数[<font color='red'>"&b1&"</font>]，制程超耗笔数[<font color='red'>"&a1&"</font>]，一分厂制程超耗率为：[<font color='red'>"&formatnumber(c1,2)&"%</font>]，二分厂制程异常笔数[<font color='red'>"&b2&"</font>]，制程超耗笔数[<font color='red'>"&a2&"</font>]，二分厂制程超耗率为：[<font color='red'>"&formatnumber(c2,2)&"%</font>]，三分厂制程异常笔数[<font color='red'>"&b3&"</font>]，制程超耗笔数[<font color='red'>"&a3&"</font>]，三分厂制程超耗率为：[<font color='red'>"&formatnumber(c3,2)&"%</font>]，娄桥分厂制程异常笔数[<font color='red'>"&b4&"</font>]，制程超耗笔数[<font color='red'>"&a4&"</font>]，娄桥分厂制程超耗率为：[<font color='red'>"&formatnumber(c4,2)&"%</font>]"
end if
  end if
  rs.close
  set rs=nothing
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>制程超耗率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="ProductConsum.asp?Result=Search">
		...
		<a href="ProductConsum.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("损失200以上金额的事故")'>损失200以上金额的事故</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td width="50" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>项目</strong></font></td>
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>不合格现象</strong></font></td>
    <td width="30" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>损失金额</strong></font></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">负责人</font></strong></td>
    <td width="80" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">确认人</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">现象分类</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">原因分类</font></strong></td>
 ...
 <%
  ...
  dim datafrom'数据表名
      datafrom="manusys_ProductConsum"
  dim datawhere'数据条件
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("ftext")&"</td>"
      Response.Write "<td nowrap>"&rs("ftext1")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("fdecimal1")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("fbase")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("fuser")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("ftext4")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("ftext5")&"</td>" & vbCrLf
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'PCreply',"&rs("SerialNum")&")"">"&rs("a20")&"</td>" & vbCrLf
