...
sqlstr="t_DeliverCheck"


function PlaceFlag()
  dim rs,sql,sqlstr2'sql语句
  if Result="Search" then
	sqlstr2="select sum(FinishRemakeOne) as idCount2 from manusys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    unReachsum=rs("idCount2")
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，成品返工件数[<font color='red'>"&unReachsum&"</font>]"
  else
	sqlstr2="select top 1 FinishRemakeOne as idCount2 from manusys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    unReachsum=rs("idCount2")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，成品返工件数[<font color='red'>"&unReachsum&"</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>成品返工件数信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="FinishRemake.asp?Result=Search">
          <td nowrap> 产品检索：从
          <script language=javascript> 
		...
		<a href="FinishRemake.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("返工返修")'>返工返修</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td width="30" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>产品编号</strong></font></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">数量</font></strong></td>
    <td width="80" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">箱数</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">判定</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">不良描述</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">处理方式</font></strong></td>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("fbase1")&"</td>" & vbCrLf
      ...
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'FRreply',"&rs("SerialNum")&")"">"&rs("a20")&"</td>" & vbCrLf
