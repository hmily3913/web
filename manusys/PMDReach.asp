...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(PMDROne) as idCount1,sum(unPMDROne) as idCount2 from manusys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    if unReachsum=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，共计划笔数为0"
	else
    Reachper=Reachsum/unReachsum*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，共计划笔数[<font color='red'>"&unReachsum&"</font>]，生管满足营销交期数[<font color='red'>"&Reachsum&"</font>]，生管交期达成率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
	sqlstr2="select top 1 PMDROne as idCount1,unPMDROne as idCount2,PMDReachOne from manusys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("PMDReachOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，共计划笔数[<font color='red'>"&unReachsum&"</font>]，生管满足营销交期数[<font color='red'>"&Reachsum&"</font>]，生管交期达成率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>生管交期达成率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="PMDReach.asp?Result=Search">
		...
		<a href="PMDReach.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("超交期列表")'>超交期列表</a>
		|<input name='output' type='button' class='button'  id='output' value='引出' onClick='window.open("PMDReachPrint.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1","Print","","false")'>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">责任部门</font></strong></td>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    sql="select *,left(ReplyText,10) as a11 from "& datafrom &" where SerialNum in("& sqlid &") "&taxis
    set rs=server.createobject("adodb.recordset")
    rs.open sql,connzxpt,0,1
	dim bgcolors
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'PMDRreply',"&rs("SerialNum")&")"">"&rs("a11")&"</td>" & vbCrLf
