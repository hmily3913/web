...
<%
if Instr(session("AdminPurview"),"|602,")=0 then 
...
"and a.fcombobox1 = '模具检修' and a.FBillStatus='制作'  "&_
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(MoldmOne) as idCount1,sum(unMoldmOne) as idCount2 from technologysys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    if unReachsum=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，模具制造笔数为0"
	else
    Reachper=Reachsum/unReachsum*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，模具制造笔数[<font color='red'>"&unReachsum&"</font>]，模具制造及时笔数[<font color='red'>"&Reachsum&"</font>]，模具制造及时率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
	sqlstr2="select top 1 MoldmOne as idCount1,unMoldmOne as idCount2,MoldmakeOne from technologysys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("MoldmakeOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，模具制造笔数[<font color='red'>"&unReachsum&"</font>]，模具制造及时笔数[<font color='red'>"&Reachsum&"</font>]，模具制造及时率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>模具制作及时率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="Moldmake.asp?Result=Search">
		...
		<a href="Moldmake.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("模具维修延误")'>模具制作延误</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <%
  ...
  dim datafrom'数据表名
      datafrom="technologysys_Moldmake"
  dim datawhere'数据条件
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'MMreply',"&rs("SerialNum")&")"">"&rs("a20")&"</td>" & vbCrLf
