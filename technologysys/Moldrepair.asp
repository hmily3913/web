...
<%
if Instr(session("AdminPurview"),"|601,")=0 then 
...
"and a.fcombobox1 = '模具检修' and a.FBillStatus='维修'  "&_
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(MoldrepOne) as idCount1,sum(unMoldrepOne) as idCount2 from technologysys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    if unReachsum=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，模具维修笔数为0"
	else
    Reachper=Reachsum/unReachsum*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，模具维修笔数[<font color='red'>"&unReachsum&"</font>]，模具维修及时笔数[<font color='red'>"&Reachsum&"</font>]，模具维修及时率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
	sqlstr2="select top 1 MoldrepOne as idCount1,unMoldrepOne as idCount2,MoldrepairOne from technologysys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("MoldrepairOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，模具维修笔数[<font color='red'>"&unReachsum&"</font>]，模具维修及时笔数[<font color='red'>"&Reachsum&"</font>]，模具维修及时率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>模具维修及时率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="Moldrepair.asp?Result=Search">
		...
		<a href="Moldrepair.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("模具维修延误")'>模具维修延误</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td width="50" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>部门</strong></font></td>
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>申请人</strong></font></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>设备编号</strong></font></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">设备名称</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">计划日期</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">完成日期</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">备注</font></strong></td>
 ...
 <%
  ...
  dim datafrom'数据表名
      datafrom="technologysys_Moldrepair"
  dim datawhere'数据条件
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'MRreply',"&rs("SerialNum")&")"">"&rs("a20")&"</td>" & vbCrLf
