...
<%
if Instr(session("AdminPurview"),"|402,")=0 then 
...
sqlstr="select a.fid,a.fbillno,a.fdate1, t_Supplier.Fname as name1,a.fbase1, "&_
"t_ICItem.fname as name2,a.ftext,a.fdecimal,a.ftext1,a.fcombobox,a.fcombobox2, "&_
"a.fdecimal2,a.ftext2,a.ftext3,a.FDecimal1 "&_
"from t_supplycheck as a left join  "&_
"t_Supplier on t_Supplier.FItemID=a.fbase left join "&_
"t_ICItem on t_ICItem.FItemID=a.fbase1 "&_
"where a.fdate1>='"&StartDate&"' and a.fdate1<='"&EndDate&"' and a.fcombobox2='不合格'"


function PlaceFlag()
  dim rs,sql,sqlstr2'sql语句
  if Result="Search" then
	sqlstr2="select sum(UnQualifiMtrOne) as idCount1,sum(unUnQualifiMtrOne) as idCount2 from qcsys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    ...
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，共有不合格来料笔数[<font color='red'>"&unReachsum&"</font>]，处理笔数[<font color='red'>"&Reachsum&"</font>]，不合格来料处理率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
	sqlstr2="select top 1 UnQualifiMtrOne as idCount1,unUnQualifiMtrOne as idCount2,UnQualifiMtrDealOne from qcsys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("UnQualifiMtrDealOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，共有不合格来料笔数[<font color='red'>"&unReachsum&"</font>]，处理笔数[<font color='red'>"&Reachsum&"</font>]，不合格来料处理率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>不合格来料处理率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="UnQualifiMtrDeal.asp?Result=Search">
		...
		<a href="UnQualifiMtrDeal.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("不合格未处理列表")'>不合格未处理列表</a><font color="#0000FF">
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <%
  ...
  dim datafrom'数据表名
      datafrom="qcsys_UnQualifiMtrDeal"
  dim datawhere'数据条件
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'UQMDreply',"&rs("SerialNum")&")"">"&rs("a1")&"</td>" & vbCrLf
