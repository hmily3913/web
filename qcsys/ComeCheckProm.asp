...
<%
if Instr(session("AdminPurview"),"|403,")=0 then 
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>进料检验的及时率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="ComeCheckProm.asp?Result=Search">
		...
		<a href="ComeCheckProm.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("超交期列表")'>延迟检验列表</a>
		|<input name='output' type='button' class='button'  id='output' value='引出' onClick='window.open("ComeCheckPromPrint.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1","Print","","false")'>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'CCPreply',"&rs("SerialNum")&")"">"&rs("a11")&"</td>" & vbCrLf
