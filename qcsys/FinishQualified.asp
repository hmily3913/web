...
<%
if Instr(session("AdminPurview"),"|401,")=0 then 
...
sqlstr="select a.fid,a.fbillno,a.fdate,a.ftext,a.fqty,a.FDecimal,a.FComboBox,a.FComboBox1, "&_
"t_emp.fname as name1,t_ICItem.fname as name2 from  "&_
"t_DeliverCheck a left join t_emp "&_
"on a.fbase=t_emp.fitemid left join t_ICItem  "&_
"on t_ICItem.FItemID=a.fbase1 "&_
"where a.fdate>='"&StartDate&"' and a.fdate<='"&EndDate&"' and a.fuser>0"


function PlaceFlag()
  dim rs,sql,sqlstr2'sql语句
  if Result="Search" then
	sqlstr2="select sum(FinishQOne) as idCount1,sum(unFinishQOne) as idCount2 from qcsys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    if unReachsum=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，共有成品检验笔数为0"
	else
    Reachper=Reachsum/unReachsum*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，共有成品检验笔数[<font color='red'>"&unReachsum&"</font>]，合格笔数[<font color='red'>"&Reachsum&"</font>]，成品合格率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
	sqlstr2="select top 1 FinishQOne as idCount1,unFinishQOne as idCount2,FinishQualifiedOne from qcsys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("FinishQualifiedOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，共有成品检验笔数[<font color='red'>"&unReachsum&"</font>]，合格笔数[<font color='red'>"&Reachsum&"</font>]，成品合格率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>出货合格率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="FinishQualified.asp?Result=Search">
		...
		<a href="FinishQualified.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("不合格列表")'>不合格列表</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">件数</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">检验数量</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">不合格数量</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">判定</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">处理方式</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">描述</font></strong></td>
 ...
 <%
  ...
  dim datafrom'数据表名
      datafrom="qcsys_FinishQualified"
  dim datawhere'数据条件
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("FDecimal3")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("FComboBox")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("FComboBox1")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("FText1")&"</td>" & vbCrLf
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'FQreply',"&rs("SerialNum")&")"">"&rs("a1")&"</td>" & vbCrLf
