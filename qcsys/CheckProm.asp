...
<%
if Instr(session("AdminPurview"),"|406,")=0 then 
...
sqlstr="select a.fid,a.FBillNo,a.ftext as 送检样品,a.ftext1 as 产品型号,a.fdate as 申请日期, "&_
"Department1.fname as 申请部门,Department2.fname as 送检部门, "&_
"User1.fname as 制单人,a.FDate1 as 预计完成日期,a.FDate3 as 实际完成日期,  "&_
"datediff(d,a.FDate1,isnull(a.FDate3,getdate())) as diffdays  "&_
"from t_testRequest as a LEFT OUTER JOIN dbo.t_Department AS Department1  "&_
"ON Department1.FItemID = t_testRequest.Fbase LEFT OUTER JOIN dbo.t_Department AS Department2  "&_
"ON Department2.FItemID = t_testRequest.Fbase1 LEFT OUTER JOIN dbo.t_User AS User1  "&_
"ON t_testRequest.Fbiller = User1.FUserID "&_
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(CheckPOne) as idCount1,sum(unCheckPOne) as idCount2 from qcsys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    if unReachsum=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，共有检测申请笔数为0"
	else
    Reachper=Reachsum/unReachsum*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，共有检测申请笔数[<font color='red'>"&unReachsum&"</font>]，检测及时笔数[<font color='red'>"&Reachsum&"</font>]，检测及时率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
	sqlstr2="select top 1 CheckPOne as idCount1,unCheckPOne as idCount2,CheckPromOne from qcsys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("CheckPromOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，共有检测申请笔数[<font color='red'>"&unReachsum&"</font>]，检测及时笔数[<font color='red'>"&Reachsum&"</font>]，检测及时率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>检测及时率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="CheckProm.asp?Result=Search">
		...
		<a href="CheckProm.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("检测延迟列表")'>检测延迟列表</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td width="50" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>送检样品</strong></font></td>
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>产品型号</strong></font></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">申请部门</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">送检部门</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">制单人</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">预计完成日期</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">实际完成日期</font></strong></td>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("申请日期")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("FBillNo")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("送检样品")&"</td>"
      Response.Write "<td nowrap>"&rs("产品型号")&"</td>" & vbCrLf

      Response.Write "<td nowrap>"&rs("申请部门")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("送检部门")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("制单人")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("预计完成日期")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("实际完成日期")&"</td>" & vbCrLf
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'CPreply',"&rs("SerialNum")&")"">"&rs("a1")&"</td>" & vbCrLf
