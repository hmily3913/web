...
<%
if Instr(session("AdminPurview"),"|405,")=0 then 
...
sqlstr="[Q-顾客抱怨调查处理报告单单头表]"


function PlaceFlag()
  dim rs,sql,sqlstr2'sql语句
  if Result="Search" then
	sqlstr2="select sum(CustomComplainOne) as idCount1 from qcsys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，客户投诉笔数[<font color='red'>"&Reachsum&"</font>]"
  else
	sqlstr2="select top 1 CustomComplainOne from qcsys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachper=rs("CustomComplainOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，客户投诉笔数[<font color='red'>"&Reachper&"</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>客户投诉信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="CustomComplain.asp?Result=Search">
		...
		<a href="CustomComplain.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("所有客诉信息")'>所有客诉信息</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>客诉日期</strong></font></td>
    <td width="80" height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>单号</strong></font></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>订单号</strong></font></td>
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>品名规格</strong></font></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">交货数</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">退货数</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">业务员</font></strong></td>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("客诉日期")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("id")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("工厂订单号")&"</td>"
      Response.Write "<td nowrap>"&rs("品名及规格")&"</td>" & vbCrLf

      Response.Write "<td nowrap>"&rs("交货数")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("退货数")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("业务人员姓名")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("损失金额")&"</td>" & vbCrLf
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'CCreply',"&rs("SerialNum")&")"">"&rs("a1")&"</td>" & vbCrLf
