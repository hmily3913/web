...
function pageN(){
	...
	//加载list内容，ajax提交
	$('#listDiv').load("RepairApplicationDetails.asp #listtable",{
...
function showpadd(obj,sid){
	$('#loading-one').empty().append('页面载入中...').parent().fadeIn('slow');
	//加载list内容，ajax提交
	$('#addShowDiv').load("RepairApplicationDetails.asp #AddandEditdiv",{
...
//处理提交事务
function toSubmit(){
  if($("input[name=RepairType]:checked").val()===undefined){alert('维修类型不能为空,请检查');return false;}
  if($("#DeviceName").val()==''){alert('设备名称不能为空,请检查');return false;}
	if($('#detailType').val()=='接收'){
		var txt='当前操作：接收<br>你可以选择驳回或者接收，如果有说明请在先输入相关说明描述。<br>';
		$.prompt(txt,{
			buttons: { 同意: 'Check', 驳回: 'UnCheck' },
			submit:function(v,m,f){ 
				if(v=='Check'){
					$('#detailType').val('接收');
					$.post('RepairApplicationDetails.asp?showType=DataProcess',$("#AddForm").serialize(),function(data){
...
function getInfo(obj){
  if($("#"+obj).val()==''){alert("对应编号不能为空！");return false;}
	$.get("RepairApplicationDetails.asp", { showType: "getInfo",detailType: obj, InfoID: $("#"+obj).val() },
	   function(data){
...
<font color="#FF0000"><strong>维修申请处理进度汇总</strong></font>
...
<font style="background-color:#ff99ff"><a href="javascript:$('#flag4search').val('2');pageN(0);">维修中</a></font>&nbsp;
<font style="background-color:#66ff66"><a href="javascript:$('#flag4search').val('3');pageN(0);">处理完毕</a></font>&nbsp;
<input type="hidden" name="flag4search" id="flag4search" value="">
<select id="type4search" onChange="pageN(0)" style="font-size:12px;height:13px; width:70px; z-index:1"><option value="">所有类型</option><option value="网络维修">网络维修</option><option value="行政维修">行政维修</option></select>
