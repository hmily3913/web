...
<%
  if sqlid<>"" then'如果记录总数=0,则不处理
    ...
    sql="select SerialNum,Title,Biller,BillDate from "& datafrom &" where SerialNum in("& sqlid &") "&taxis
    set rs=server.createobject("adodb.recordset")
    rs.open sql,connzxpt,1,1
...
elseif showType="DataProcess" then 
  ...
  if detailType="AddNew" then
  	if  Instr(session("AdminPurviewFLW"),"|506.1,")>0 then
		set rs = server.createobject("adodb.recordset")
		sql="select * from oa_Announce"
  ...
  elseif detailType="Edit"  then
	...
	sql="select * from oa_Announce where SerialNum="&SerialNum
	rs.open sql,connzxpt,1,3
  ...
  elseif detailType="Delete" then
    ...
    SerialNum=request("SerialNum")
		connzxpt.Execute("Delete from oa_Announce where SerialNum in ("&SerialNum&")")
		response.write "###"
  end if
...
if detailType="SerialNum" then
	...
	sql="select * from oa_Announce where SerialNum ="&InfoID
	set rs=server.createobject("adodb.recordset")
	rs.open sql,connzxpt,1,1
    if rs.bof and rs.eof then
