...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    sql="select [员工代号],[姓名],[权限_FLW] from ["& datafrom &"] where 员工代号 in("& sqlid &") "&taxis
    set rs=server.createobject("adodb.recordset")
    rs.open sql,conn,0,1
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td width='38' nowrap><a href='FLW_PurviewEdit.asp?Result=Modify&ID="&rs("员工代号")&"' onClick='changeAdminFlag(""修改管理员"")'><font color='#330099'>修改</font></a></td>" & vbCrLf
