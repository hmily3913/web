...
'显示版本号，版本日期=======================================
dim objSvn,WebRevision,WebDate,WebMaxRev
Set objSvn = CreateObject("SubWCRev.object")
objSvn.GetWCInfo Server.MapPath("."), 1, 1
WebRevision=objSvn.Revision'当前版本号
WebDate=objSvn.Date'版本日期
WebMaxRev=objSvn.MaxRev'最大可更新版本号
set objSvn=Nothing
'========
%>
<table name="Trans" id="Trans" width="100%" height="24" border="0" cellpadding="0" cellspacing="0" style="BORDER-BOTTOM: #333333 1px solid;font-family:宋体;font-size:12px;color: #333333;">
  <tr>
    <td width="220" nowrap >版本号：<%= WebRevision %>[<%= WebDate %>]</td>
    <td width="180" nowrap><%=session("AdminName")%>：[<%=session("UserName")%>]</td>
    <td width="36" nowrap>位置：</td>
    <td width="120" nowrap>报表首页</td>
...
//-----------------------------------------------农历算法结束-------------------------------------------------------
var str = GetLunarDateString(new Date());

         setInterval("document.getElementById('DateTime').innerHTML=new Date().toLocaleString()+' 星期'+'日一二三四五六'.charAt(new Date().getDay())+' '+str;",1000);
      </script>
    </td>
  </tr>
</table>