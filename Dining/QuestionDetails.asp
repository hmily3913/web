...
<%
if Instr(session("AdminPurviewFLW"),"|604,")=0 then 
...
<%
  if sqlid<>"" then'如果记录总数=0,则不处理
    ...
    sql="select SerialNum,ClassType,Question_type,Question_content,Question_must,CancelFlag,UseFlag from "& datafrom &" where SerialNum in("& sqlid &") "&taxis
    set rs=server.createobject("adodb.recordset")
    rs.open sql,connzxpt,1,1
...
elseif showType="DataProcess" then 
  ...
  if detailType="AddNew" then
  	if  Instr(session("AdminPurviewFLW"),"|604.1,")>0 then
		set rs = server.createobject("adodb.recordset")
		sql="select * from Dining_Question"
		rs.open sql,connzxpt,1,3
		rs.addnew
		rs("Question_type")=request.Form("Question_type")
		rs("Question_answer")=request.Form("Question_answer")
		rs("Question_content")=request.Form("Question_content")
  ...
  elseif detailType="Edit"  then
    ...
    SerialNum=request("SerialNum")
		sql="select * from Dining_Question where SerialNum="&SerialNum
		rs.open sql,connzxpt,1,3
  ...
  elseif detailType="Delete" then
    ...
    SerialNum=request("SerialNum")
		connzxpt.Execute("Delete from Dining_Question where UseFlag=0 and SerialNum in ("&SerialNum&")")
		response.write "###"
  elseif detailType="Forbid" then
    ...
    SerialNum=request("SerialNum")
		connzxpt.Execute("Update Dining_Question set CancelFlag=1 where SerialNum in ("&SerialNum&")")
		response.write "禁用成功！"
  elseif detailType="Forbid" then
		if Instr(session("AdminPurviewFLW"),"|604.1,")=0 then
			response.write ("你没有权限进行此操作！")
    ...
    SerialNum=request("SerialNum")
		connzxpt.Execute("Update Dining_Question set CancelFlag=0 where SerialNum in ("&SerialNum&")")
		response.write "启用成功！"
  end if
...
if detailType="SerialNum" then
	...
	sql="select * from Dining_Question where SerialNum ="&InfoID
	set rs=server.createobject("adodb.recordset")
	rs.open sql,connzxpt,1,1
    if rs.bof and rs.eof then
