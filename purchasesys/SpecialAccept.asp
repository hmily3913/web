...
<%
if Instr(session("AdminPurview"),"|204,")=0 then 
...
"t_ICItem.fname as name2,a.ftext,a.fdecimal,a.ftext1,a.fcombobox, "&_
...
"where a.fdate1>='"&StartDate&"' and a.fdate1<='"&EndDate&"' and a.FComboBox='让步接收'"


function PlaceFlag()
  dim rs,sql,sqlstr2'sql语句
  if Result="Search" then
	sqlstr2="select sum(SpecialAcceptOne) as idCount1 from purchasesys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，共有特采笔数[<font color='red'>"&Reachsum&"</font>]，"
  else
	sqlstr2="select top 1 SpecialAcceptOne from purchasesys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachper=rs("SpecialAcceptOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，共有特采笔数：[<font color='red'>"&Reachper&"</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>特采件数信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="SpecialAccept.asp?Result=Search">
...
<a href="SpecialAccept.asp?Result=Search&Keyword=all&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("全部特采件")'>全部特采件</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>处理日期</strong></font></td>
    ...
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>处理单号</strong></font></td>
    ...
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">不良数量</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">处理数量</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">不良描述</font></strong></td>
 ...
 <%
  ...
  dim datafrom'数据表名
      datafrom="purchase_SpecialAccept"
  dim datawhere'数据条件
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'SAreply',"&rs("SerialNum")&")"">"&rs("a1")&"</td>" & vbCrLf
