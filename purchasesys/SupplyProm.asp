...
<%
if Instr(session("AdminPurview"),"|203,")=0 then 
...
&nbsp;<input name="submitSaveEdit" type="button" class="button sv"  id="submitSaveEdit" value="保存" style="WIDTH: 80;" onClick="SaveEdit()">
&nbsp;<input type="button" class="button cf" value="确定" style="WIDTH: 80;" onClick="confirmClick('SPreplycf',1)">
&nbsp;<input type="button" class="button cf" value="否定" style="WIDTH: 80;" onClick="confirmClick('SPreplycf',0)">
</td>
</tr>
</table>
</form>
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>供货及时率</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="SupplyProm.asp?Result=Search">
		...
		<a href="SupplyProm.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("超交期列表")'>超交期列表</a>
		|<input name='output' type='button' class='button'  id='output' value='引出' onClick='window.open("SupplyPromPrint.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1","Print","","false")'>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap onDblClick=""SAClickTd(this,'SPreply',"&rs("SerialNum")&");$('.cf').hide();$('.sv').show();"" title='双击进行回复及确认！'>"&rs("a1")&"</td>" & vbCrLf
      Response.Write "<td nowrap onDblClick=""SAClickTd(this,'SPreply',"&rs("SerialNum")&");$('.cf').show();$('.sv').hide();"" title='双击进行回复及确认！'>"&rs("replyConfirm")&"</td>" & vbCrLf
      Response.Write "</tr>" & vbCrLf
	  rs.movenext
    wend
    Response.Write "<tr>" & vbCrLf
    Response.Write "<td colspan='15' nowrap  bgcolor='#EBF2F9'>&nbsp;</td>" & vbCrLf
...
'-----------------------------------------------------------
  Response.Write "<tr>" & vbCrLf
  Response.Write "<td colspan='17' nowrap  bgcolor='#D7E4F7'>" & vbCrLf
