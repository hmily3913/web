...
<%
if Instr(session("AdminPurview"),"|201,")=0 then 
...
sqlstr="SELECT     TOP (100) PERCENT v1.FDate AS 日期, v1.FBillNo AS 外购入库单号, u1.FEntryID AS 行号, Supplier.Fname AS 供应商名称,  "&_
"                      v1.FHeadSelfA0142 AS 送货单编号, u1.FBatchNo AS 批号, ICItem.FNumber AS 物料代码, ICItem.FName AS 物料名称, Unit1.FName AS 单位,  "&_
"                      u1.FQty AS 基本实收数量, u1.FQtyMust AS 基本应收数量, u1.FAuxQtyMust AS 应收数量, u1.FAuxQty AS 实收数量, Unit2.FName AS 基本单位,  "&_
"                      u1.FPlanMode AS 计划模式, u1.FMTONo AS 计划跟踪号,  "&_
"                      u1.FEntrySelfA0155 AS 工厂订单号, u1.FEntrySelfA0159 AS 销售订单号, v1.FCheckDate AS 审核日期, u1.FSourceTranType AS 源单类型,  "&_
"                      u1.FSourceBillNo AS 源单编号, u1.FNote AS 备注,  (LTRIM(RTRIM(STR(u1.FInterID)))+'t'+  "&_
"                      + LTRIM(RTRIM(STR(u1.FEntryID)))) AS id "&_
"FROM         dbo.ICStockBillEntry AS u1 INNER JOIN "&_
"                      dbo.ICStockBill AS v1 ON u1.FInterID = v1.FInterID AND v1.FTranType = 1 INNER JOIN "&_
"                      dbo.t_Supplier AS Supplier ON v1.FSupplyID = Supplier.FItemID LEFT OUTER JOIN "&_
...
"where v1.FDate>='"&StartDate&"' and v1.FDate<='"&EndDate&"' "&_
"ORDER BY 外购入库单号, 行号"
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(ReturnOne) as idCount1,sum(unReturnOne) as idCount2 from purchasesys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    ...
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，共有采购笔数[<font color='red'>"&unReachsum&"</font>]，退货笔数[<font color='red'>"&Reachsum&"</font>]，退货率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
	sqlstr2="select top 1 ReturnOne as idCount1,unReturnOne as idCount2,ReturnRateOne from purchasesys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("ReturnRateOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，共有采购笔数[<font color='red'>"&unReachsum&"</font>]，退货笔数[<font color='red'>"&Reachsum&"</font>]，退货率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>退货率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="ReturnRate.asp?Result=Search">
		...
		<a href="ReturnRate.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("退货列表")'>退货列表</a><font color="#0000FF"></a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td width="80" height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>供应商名称</strong></font></td>
    ...
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">应退数量</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">实退数量</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">审核日期</font></strong></td>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("供应商名称")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("外购入库单号")&"</td>"
      ...
      Response.Write "<td nowrap>"&rs("基本应收数量")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("基本实收数量")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("审核日期")&"</td>" & vbCrLf
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'RRreply',"&rs("SerialNum")&")"">"&rs("a1")&"</td>" & vbCrLf
