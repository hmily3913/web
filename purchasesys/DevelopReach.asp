...
<%
if Instr(session("AdminPurview"),"|206,")=0 then 
...
sqlstr="select a.fbillno,a.fdate,b.ftext,b.ftext1,b.ftext2, "&_
"b.fdecimal,b.fdecimal1,b.fdate5,b.ftext3,b.fdate3,b.fdate4, (LTRIM(RTRIM(STR(b.fid)))+'t'+  "&_
"                      + LTRIM(RTRIM(STR(b.FEntryID)))) AS fid,"&_
"datediff(d,b.fdate3,isnull(b.fdate3,getdate())) as diffdays "&_
"from t_develop_buy a inner join t_develop_buy_details b on a.fid=b.fid "&_
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(DevelopOne) as idCount1,sum(unDevelopOne) as idCount2 from purchasesys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    if unReachsum=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，共有材料开发笔数为0"
	else
    Reachper=Reachsum/unReachsum*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，共有材料开发笔数[<font color='red'>"&unReachsum&"</font>]，达成笔数[<font color='red'>"&Reachsum&"</font>]，材料开发达成率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
	sqlstr2="select top 1 DevelopOne as idCount1,unDevelopOne as idCount2,DevelopReachOne from purchasesys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("DevelopReachOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，共有材料开发笔数[<font color='red'>"&unReachsum&"</font>]，达成笔数[<font color='red'>"&Reachsum&"</font>]，材料开发达成率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>材料开发达成率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="DevelopReach.asp?Result=Search">
		...
		<a href="DevelopReach.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("未按期达成列表")'>未按期达成列表</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>申购日期</strong></font></td>
    <td width="80" height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>单号</strong></font></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>品名</strong></font></td>
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>规格</strong></font></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">品质要求</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">申请数量</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">现场存货</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">需求日期</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">回复日期</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">到货日期</font></strong></td>
 ...
 <%
  ...
  dim datafrom'数据表名
      datafrom="purchase_DevelopReach"
  dim datawhere'数据条件
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("fdate5")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("fdate4")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("fdate3")&"</td>" & vbCrLf
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'DRreply',"&rs("SerialNum")&")"">"&rs("a1")&"</td>" & vbCrLf
