...
<% 
  ...
  '获取记录总数
  sql="select count(finterid) as idCount from "& datafrom &" " & datawhere
...
//如果回复内容为空，不提交
 if($('#ReplyText').val()!=''){
  jQuery.get("FlwAjaxFunction.asp", { "key": "update"+$("#Keyword").val(), "FItemid": $("#Finterid").val(),"ReplyText":$('#ReplyText').val() },
   function(data){
		if(data.indexOf("###")>-1){
			...
			//实时改变提交后背景颜色
			if($("#Keyword").val()=="PCreply")curTd.parentNode.bgColor="#ffff66";
			else if($("#Keyword").val()=="MCreply")curTd.parentNode.bgColor="#ff99ff";
			else if($("#Keyword").val()=="STreply")curTd.parentNode.bgColor="#66ff66";
			else if($("#Keyword").val()=="SEreply")curTd.parentNode.bgColor="#6666ff";
		}
		$("#ReplyDiv").hide("slow");
   });
  }else{
...
function pageN(){
	...
	var tnow=new Date();
	//加载list内容，ajax提交
	$('#listDiv').load("IcomFlwDetails.asp #listtable",{Page:arr[0],ntime:tnow},function(response, status, xhr){
	  if (status =="success") {
...
<font color="#FF0000"><strong>生产任务计划未达成确认</strong></font>
