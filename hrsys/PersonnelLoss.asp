...
<%
if Instr(session("AdminPurview"),"|901,")=0 then 
...
sqlstr="  SELECT he.Code as id,he.name,empa.HRMS_UserField_3,emw.EnterDate,emw.InDueDate, "&_
"case when he.Status=1 then 0 else 1 end as 离职,  "&_
"case when he.Status=1 then null when he.Status=2 then  wbd.DimissionDate end as 离职日期, "&_
"case when he.Status=1 then '在职' when he.Status=2 then wbd.DimissionType end as 员工状态,wbd.Description  "&_
"FROM Wf_biz_DimissionInfo wbd inner JOIN  "&_
"HM_Employees he ON wbd.EM_ID = he.EM_ID inner join  "&_
"(select EM_ID,max(DimissionDate) as DimissionDate from Wf_biz_DimissionInfo group by EM_ID)b  "&_
"on b.EM_ID=wbd.EM_ID and b.DimissionDate=wbd.DimissionDate  left outer join  "&_
"HM_EmployeesAddInfo empa ON he.EM_ID = empa.EM_ID left outer join  "&_
"HM_EM_WorkInfo emw ON he.EM_ID = emw.EM_ID "&_
"where  he.Status<>1 and ( "&_
"(empa.HRMS_UserField_3=1 and datediff(dd,emw.EnterDate,wbd.DimissionDate)>7)  "&_
"or (empa.HRMS_UserField_3>1 and datediff(dd,emw.InDueDate,wbd.DimissionDate)>0)) "&_
"union  "&_
"SELECT he.Code as id,he.name,empa.HRMS_UserField_3,emw.EnterDate,emw.InDueDate, "&_
"case when he.Status=1 then 0 else 1 end as 离职,  "&_
"case when he.Status=1 then null when he.Status=2 then  wbf.FireDate end as 离职日期 ,  "&_
"case when he.Status=1 then '在职' when he.Status=2 then  wbf.FireType end as 员工状态, wbf.Description  "&_
"FROM Wf_biz_FireInfo wbf inner JOIN  "&_
"HM_Employees he ON wbf.EM_ID = he.EM_ID inner join ( "&_
"select EM_ID,max(FireDate) as FireDate from Wf_biz_FireInfo group by EM_ID)b  "&_
"on b.EM_ID=wbf.EM_ID and b.FireDate=wbf.FireDate left outer join  "&_
"HM_EmployeesAddInfo empa ON he.EM_ID = empa.EM_ID left outer join  "&_
"HM_EM_WorkInfo emw ON he.EM_ID = emw.EM_ID "&_
"where he.Status<>1 and ( "&_
"(empa.HRMS_UserField_3=1 and datediff(dd,emw.EnterDate,wbf.FireDate)>7)  "&_
"or (empa.HRMS_UserField_3>1 and datediff(dd,emw.InDueDate,wbf.FireDate)>0)) "
'----------------------------------------------------------------------------------
sqlstr2=""
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(PersonnelLLDOne) as a1,sum(unPersonnelLLDOne) as b1,sum(PersonnelLLQOne) as a2,sum(unPersonnelLLQOne) as b2,sum(1) as c from hrsys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    ...
    d1=a1/b1*100
    d2=a2/b2*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，蓝道平均在职人数[<font color='red'>"&c1&"</font>]，流失人数[<font color='red'>"&a1&"</font>]，蓝道人员流失率为：[<font color='red'>"&formatnumber(d1,2)&"%</font>]，娄桥平均在职人数[<font color='red'>"&c2&"</font>]，流失人数[<font color='red'>"&a2&"</font>]，娄桥人员流失率为：[<font color='red'>"&formatnumber(d2,2)&"%</font>]"
  else
	sqlstr2="select top 1 PersonnelLLDOne as a1,unPersonnelLLDOne as b1,PersonnelLLQOne as a2,unPersonnelLLQOne as b2,PersonnelLossLDOne as c1,PersonnelLossLQOne as c2 from hrsys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    a1=rs("a1")
    c1=rs("b1")
    ...
    d1=rs("c1")
    d2=rs("c2")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，蓝道平均在职人数[<font color='red'>"&c1&"</font>]，流失人数[<font color='red'>"&a1&"</font>]，蓝道人员流失率为：[<font color='red'>"&formatnumber(d1,2)&"%</font>]，娄桥平均在职人数[<font color='red'>"&c2&"</font>]，流失人数[<font color='red'>"&a2&"</font>]，娄桥人员流失率为：[<font color='red'>"&formatnumber(d2,2)&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>流失人员率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="PersonnelLoss.asp?Result=Search">
          <td nowrap> 产品检索：从<input type="text" class="textfield" style="width:80px" id="start_date" name="start_date" value="<%=StartDate%>"/>

          &nbsp;到<input type="text" class="textfield" style="width:80px" id="end_date" name="end_date" value="<%=EndDate%>" /><input name="submitSearch" type="submit" class="button" value="检索">(为保证查询速度时间尽量不超过3个月)
          </td>
        </form>
		<%if Result="Search" then%>
        <td align="right" nowrap>查看：
		<a href="PersonnelLoss.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("流失人员列表")'>流失人员列表</a><font color="#0000FF">
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'PLreply',"&rs("SerialNum")&")"">"&rs("a1")&"</td>" & vbCrLf
