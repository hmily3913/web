...
<%
if Instr(session("AdminPurview"),"|904,")=0 then 
...
sqlstr="select [N-培训计划表].*,datediff(d,建议最晚培训时间,isnull(日期,getdate())) as diffdays from [N-培训计划表] left outer join [N-教育训练资料单头]  "&_
"on [N-培训计划表].部门别=[N-教育训练资料单头].部门别 and [N-教育训练资料单头].训练项目=[N-培训计划表].培训需求课程名称 "&_
"where 建议最晚培训时间>='"&StartDate&"' and 建议最晚培训时间<='"&EndDate&"'"


function PlaceFlag()
  if Result="Search" then
    dim rs,sql'sql语句
  sql="select count(序号) as idCount from ("&sqlstr&") as aaa where aaa.diffdays < 0 " 
  set rs=server.createobject("adodb.recordset")
  rs.open sql,conn,0,1
  Reachsum=rs("idCount")
  sql="select count(序号) as idCount from ("&sqlstr&") as aaa" 
  set rs=server.createobject("adodb.recordset")
  rs.open sql,conn,0,1
  unReachsum=rs("idCount")
  Reachper=Reachsum/unReachsum*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，按计划开课次数为[<font color='red'>"&Reachsum&"</font>]，计划开课次数为[<font color='red'>"&unReachsum&"</font>]，开课率为[<font color='red'>"&formatnumber(Reachper,2)&"</font>]"
  else
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>开课率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="ClassRate.asp?Result=Search">
          <td nowrap> 产品检索：从
		...
		<a href="ClassRate.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("开课列表")'>已开课列表</a><font color="#0000FF">&nbsp;|&nbsp;</font><a href="ClassRate.asp?Result=Search&Keyword=all&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("所有开课计划列表")'>所有开课计划列表</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td width="80" height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>课程名称</strong></font></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>类别</strong></font></td>
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>培训目的</strong></font></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">培训岗位</font></strong></td>
    <td width="120" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">参加培训人员</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">培训方式</font></strong></td>
    <td width="120" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">建议最早培训日期</font></strong></td>
    <td width="120" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">建议最迟培训日期</font></strong></td>
  </tr>
 <%
  dim idCount'记录总数
  dim pages'每页条数
  ...
  dim datawhere'数据条件
      if Keyword="list" then
	     datawhere=" where aaa.diffdays < 0 "
	  else
		 datawhere=" "
 	  end if
  dim sqlid'本页需要用到的id
  ...
  dim rs,sql'sql语句
  '获取记录总数
  sql="select count(序号) as idCount from ("& datafrom &") as aaa " & datawhere
  set rs=server.createobject("adodb.recordset")
  rs.open sql,conn,0,1
  idCount=rs("idCount")
  '获取记录总数
  if(idcount>0) then'如果记录总数=0,则不处理
    ...
    sql="select 序号 from ("& datafrom &") as aaa " & datawhere & taxis
    set rs=server.createobject("adodb.recordset")
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    sql="select * from ("& datafrom &") as aaa where aaa.序号 in("& sqlid &") "&taxis
    set rs=server.createobject("adodb.recordset")
    rs.open sql,conn,0,1
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("培训需求课程名称")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("类别")&"</td>"
      Response.Write "<td nowrap>"&rs("培训目的")&"</td>" & vbCrLf

      Response.Write "<td nowrap>"&rs("培训岗位")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("参加培训人员")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("培训方式             （内/外训）")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("建议最早培训时间")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("建议最晚培训时间")&"</td>" & vbCrLf
