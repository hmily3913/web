...
<%
if Instr(session("AdminPurview"),"|905,")=0 then 
...
function PlaceFlag()
  if Result="Search" then
    dim rs,sql'sql语句
  sql="select isnull(sum(错误笔数),0) as idCount from ("&sqlstr&") as aaa  " 
  set rs=server.createobject("adodb.recordset")
  rs.open sql,connk3,0,1
  Reachsum=rs("idCount")
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，薪资计算错误笔数为[<font color='red'>"&Reachsum&"</font>]"
  else
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  ...
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="SalaryCalculAccur.asp?Result=Search">
          <td nowrap> 产品检索：从
...
<a href="SalaryCalculAccur.asp?Result=Search&Keyword=all&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("所有列表")'>所有列表</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td width="120" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">错误笔数</font></strong></td>
  </tr>
 <%
  dim idCount'记录总数
  dim pages'每页条数
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("错误笔数")&"</td>" & vbCrLf
