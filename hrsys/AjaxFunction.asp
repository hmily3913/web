...
elseif key ="department" then
	...
	dim wordforout
	wordforout=""
	sqlajax="select * from t_Department where Fnumber like '%"&FItemid&"%' or  fname like '%"&FItemid&"%'"
	set rsajax=server.createobject("adodb.recordset")
	rsajax.open sqlajax,connk3,0,1
...
elseif key ="getDriver" then
	'获取司机信息
	sqlajax="select b.fnumber,b.fname from z_Car a,t_emp b where a.Driver=b.fname and a.CarID='"&FItemid&"'"
	set rsajax=server.createobject("adodb.recordset")
	rsajax.open sqlajax,connk3,0,1
	if rsajax.bof and rsajax.eof then
		response.Write("!@#$")
		...
		arryword(0)=rsajax("Fnumber")
		arryword(1)=rsajax("Fname")
		response.Write(arryword(0)&"###"&arryword(1))
	end if
'=======================网络维修及时率========================
