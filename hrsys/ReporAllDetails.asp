...
elseif showType="getChart" then 
  sql="select yearnum as 年份,Period as 月份,sum(PersonnelLoss) as 整体人员流失率,sum(PersonnelLossLD) as 滨海流失率,sum(PersonnelLossLQ) as 娄桥流失率, "&_
		"sum(PersonnelLossLD1) as 一厂流失率,sum(PersonnelLossLD2) as 二厂流失率,sum(PersonnelLossLD3) as 三厂流失率,sum(RecruitmentEffRate) as 整体招聘有效率,sum(RecruitmentEffRateLD) as 滨海有效率,sum(RecruitmentEffRateLQ) as 娄桥有效率 "&_
		"from hrsys a, "&_
		"(select max(uptdate) as 月末日期 from hrsys  "&_
