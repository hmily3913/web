...
<%
if Instr(session("AdminPurview"),"|804,")=0 then 
...
sqlstr="  select  ICPurchase.FBillNo AS 发票单号, ICPurchaseEntry.FEntryID AS 发票序号, ICPurchase.FDate AS 发票日期, ICPurchase.FCheckDate AS 发票审核日期, ICStockBill.FDate AS 入库日期, ICStockBill.FCheckDate AS 入库审核日期, t_emp.FName AS 业务员, "&_                            
" dbo.t_Supplier.FNumber AS 供应商代码,  ICItem.FNumber AS 物料代码, ICItem.FName AS 物料名称, ICPurchaseEntry.fprice,ICPurchaseEntry.famount, "&_                                                                                                                                     
" unit1.FName AS 单位, ICPurchaseEntry.FAuxQty AS 数量,case when ICPurchase.FCheckDate <= convert(nvarchar(4),DATEPART(yy, dateadd(mm,1,ICStockBill.FDate))) +'-'+convert(nvarchar(4),DATEPART(mm, dateadd(mm,1,ICStockBill.FDate)))+'-2' then '及时' else '不及时' end as 是否及时 ,"&_
"(LTRIM(RTRIM(STR(ICPurchaseEntry.FInterID)))+ 't' +LTRIM(RTRIM(STR(ICPurchaseEntry.FEntryID)))) AS id "&_
"  from ICPurchaseEntry inner join ICPurchase on ICPurchaseEntry.finterid=ICPurchase.finterid inner JOIN "&_                                                                                                                                                                           
" dbo.ICStockBillEntry  on ICPurchaseEntry.FSourceInterId = dbo.ICStockBillEntry.FInterID AND ICPurchaseEntry.FSourceEntryID = dbo.ICStockBillEntry.FEntryID   inner JOIN "&_                                                                                                          
" dbo.ICStockBill ON dbo.ICStockBill.FInterID = dbo.ICStockBillEntry.FInterID AND ICPurchaseEntry.FSourceTranType = dbo.ICStockBill.FTranType INNER JOIN "&_                                                                                                                           
" dbo.t_ICItemCore AS ICItem ON ICPurchaseEntry.FItemID = ICItem.FItemID INNER JOIN "&_                                                                                                                                                                                                
" dbo.t_Base_Emp AS t_emp ON ICPurchase.FEmpID = t_emp.FItemID INNER JOIN "&_                                                                                                                                                                                                          
" dbo.t_Supplier ON ICPurchase.FSupplyID = dbo.t_Supplier.FItemID INNER JOIN "&_                                                                                                                                                                                                       
" dbo.t_MeasureUnit AS unit1 ON ICPurchaseEntry.FUnitID = unit1.FMeasureUnitID "&_
" where ICPurchase.FDate>='"&StartDate&"' and ICPurchase.FDate<='"&EndDate&"'"                                                                                                                                                                                                             
...
function PlaceFlag()
  ...
  unReachsum=rs("idCount")
	...
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，应付笔数为0"
	else
    Reachper=Reachsum/unReachsum*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，应付未及时笔数[<font color='red'>"&Reachsum&"</font>]，总比数[<font color='red'>"&unReachsum&"</font>]，供应商应付账款对账未及时率：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>供应商应付账款对账及时率</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="PayCheckPrompt.asp?Result=Search">
          <td nowrap> 产品检索：从
		...
		<a href="PayCheckPrompt.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("付款未及时列表")'>付款未及时列表</a><font color="#0000FF">&nbsp;|&nbsp;</font><a href="PayCheckPrompt.asp?Result=Search&Keyword=all&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("查全部批次")'>全部批次</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>入库日期</strong></font></td>
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>供应商代码</strong></font></td>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("供应商代码")&"</td>" & vbCrLf
