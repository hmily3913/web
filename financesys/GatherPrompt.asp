...
<%
if Instr(session("AdminPurview"),"|803,")=0 then 
...
sqlstr=" select t_rp_contact.fid as id,t_Organization.fnumber,t_Organization.fname, "&_                                                                    
" t_PayColCondition.fname as 收款条件,t_rp_contact.fyear as 年, "&_                                                          
" t_rp_contact.fperiod as 期间,t_rp_contact.ftype as 类别, "&_                                                               
" t_rp_contact.fdate as 单据日期,t_rp_contact.ffincdate as 财务日期, "&_                                                     
" t_rp_contact.fnumber as 单据编号, Department1.fname as 部门, "&_                                                           
" emp.fname as 业务员,t_rp_contact.FExchangeRate as 汇率, "&_                                                                
" case when t_rp_contact.ftype=5 then 0 else t_rp_contact.FAmountfor end as 应收金额, "&_                                    
" case when t_rp_contact.ftype=5 then 0 else t_rp_contact.FAmount end 本位币应收金额, "&_                                    
" case when t_rp_contact.ftype<>5 then 0 else t_rp_contact.FAmountfor end as 实收金额, "&_                                   
" case when t_rp_contact.ftype<>5 then 0 else t_rp_contact.FAmount end 本位币实收金额, "&_                                   
" frpdate as 应收日期 "&_                                                                                                    
" from t_rp_contact inner join "&_                                                                                           
"  t_Organization on fcustomer =t_Organization.FItemID left outer join "&_                                                   
"  t_PayColCondition on t_Organization.fpaycondition=t_PayColCondition.fid LEFT OUTER JOIN "&_                               
...
function PlaceFlag()
  if Result="Search" then
	...
	dim tem1,tem2
	sqlstr2="select sum(本位币实收金额) as idCount from ("&sqlstr&") aaa where  aaa.财务日期>='"&StartDate&"' and aaa.财务日期<='"&EndDate&"'"
  set rs=server.createobject("adodb.recordset")
  rs.open sqlstr2,connk3,0,1
  Reachsum=rs("idCount")'本期应收
	sqlstr2="select sum(本位币应收金额) as idCount from ("&sqlstr&") aaa where  aaa.应收日期<='"&EndDate&"'"
  set rs=server.createobject("adodb.recordset")
  rs.open sqlstr2,connk3,0,1
  unReachsum=rs("idCount")
	sqlstr2="select sum(本位币实收金额) as idCount from ("&sqlstr&") aaa where  aaa.财务日期<'"&StartDate&"'"
  set rs=server.createobject("adodb.recordset")
  rs.open sqlstr2,connk3,0,1
  tem1=rs("idCount")'之前实收
    if unReachsum-tem1=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，实收数为0"
	else
	tem2=unReachsum-tem1
    Reachper=Reachsum/tem2*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，共应收金额[<font color='red'>"&tem2&"</font>]，实收[<font color='red'>"&Reachsum&"</font>]，应收账款收款到账率：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>应收账款收款到账率</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="GatherPrompt.asp?Result=Search">
          <td nowrap> 产品检索：从
		...
		<a href="GatherPrompt.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("订单变更列表")'>订单变更列表</a><font color="#0000FF"></a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>客户名称</strong></font></td>
    <td width="80" height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>单据编号</strong></font></td>
    ...
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>应收本币</strong></font></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">实收本币</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">应收日期</font></strong></td>
  </tr>
 <%
  dim idCount'记录总数
  dim pages'每页条数
  ...
  dim datawhere'数据条件
      if Keyword="list" then
	     datawhere="where  aaa.应收日期>='"&StartDate&"' and aaa.应收日期<='"&EndDate&"'"
	  else
		 datawhere=""
 	  end if
  dim sqlid'本页需要用到的id
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("本位币应收金额")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("本位币实收金额")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("应收日期")&"</td>" & vbCrLf
    ...
    response.write "<tr><td height='50' align='center' colspan='7' nowrap  bgcolor='#EBF2F9'>暂无产品信息</td></tr>"
  end if
'-----------------------------------------------------------
'-----------------------------------------------------------
  Response.Write "<tr>" & vbCrLf
