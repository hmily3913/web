...
if showType="DetailsList" then 
  ...
  dim i'用于循环的整数
	...
	elseif Instr(session("AdminPurviewFLW"),"|217.3,")>0 and Depart="KD01.0005.0001" then
		datawhere=" where (Department='"&Depart&"' or Department='KD01.0001.0009' or BillerID='"&UserName&"') "
	else
...
%>		
		{"id":"<%=rs("SerialNum")%>",
		"cell":["<%=rs("SerialNum")%>","<%=rs("RegisterID")%>","<%=rs("Register")%>","<%=rs("Department")%>","<%=rs("Departmentname")%>","<%=rs("Position")%>","<%=rs("StartDate")%>","<%=rs("StartTime")%>","<%=rs("EndDate")%>","<%=rs("EndTime")%>","<%=rs("TotalDay")%>","<%=rs("TotalHour")%>","<%=tempstr%>","<%=rs("CancelFlag")%>","<%=JsonStr(rs("Reason"))%>","<%=rs("Biller")%>","<%=rs("Checker2")%>","<%=rs("Checker")%>","<%=rs("Approvaler")%>","<%=rs("Hrer")%>"]}
<%		
	    rs.movenext
		If Not rs.eof Then
		  Response.Write ","
...
elseif showType="DataProcess" then 
  ...
  if detailType="AddNew" then
		set rs = server.createobject("adodb.recordset")
		sql="select * from Financesys_Suppliereg"
  ...
  elseif detailType="Edit"  then
		set rs = server.createobject("adodb.recordset")
		sql="select * from Financesys_Suppliereg where SerialNum="&request("SerialNum")
		rs.open sql,connzxpt,1,3
	...
	elseif detailType="Check" then
		...
		sql="select * from Financesys_Suppliereg where SerialNum in ("&SerialNum&")"
		rs.open sql,connzxpt,1,3
		while (not rs.eof)
			...
			if rs("CheckFlag")=0 and (rs("Department")="KD01.0001.0018" or rs("Department")="KD01.0001.0010" or rs("Department")="KD01.0001.0007" or rs("Department")="KD01.0001.0008" or rs("Department")="KD01.0004.0001") and Instr(session("AdminPurviewFLW"),"|217.4,")>0 and cdbl(rs("Grade"))<5 then
			...
			if rs("CheckFlag")=1 and cdbl(rs("TotalDay"))<7 and (rs("SalaryType")="3.计件" or rs("SalaryType")="2.分厂月薪") then
				...
				if request("operattext")="0" then
					...
					for nnn=0 to datediff("d",rs("StartDate"),rs("EndDate"))
						...
						connkq.Execute("insert into USER_SPEDAY select userid,'"&dateadd("d",nnn,rs("StartDate"))&stime&"','"&dateadd("d",nnn,rs("StartDate"))&etime&"',"&qjlb&",'资讯平台请假单审核通过，单号："&rs("SerialNum")&"',getdate(),'Financesys_Suppliereg',"&rs("SerialNum")&" from USERINFO where ssn='"&rs("RegisterID")&"'")
					next
				end if
			elseif rs("CheckFlag")=2 then
				dim rs2,stime,etime
				...
				if rs2.eof and rs2.bof and ((cdbl(rs("TotalDay"))<7 and rs("SalaryType")="1.行政月薪") or (cdbl(rs("TotalDay"))<15 and (rs("SalaryType")="3.计件" or rs("SalaryType")="2.分厂月薪"))) then
					rs("HrerID")=UserName
					rs("Hrer")=AdminName
					rs("HrDate")=now()
					rs("CheckFlag")=4
  ...
  elseif detailType="Delete" then
    SerialNum=request("SerialNum")
		sql="select * from Financesys_Suppliereg where CheckFlag>0 and SerialNum in ("&SerialNum&")"
		set rs=server.createobject("adodb.recordset")
		rs.open sql,connzxpt,1,1
		if rs.eof then
			connzxpt.Execute("Delete from Financesys_Suppliereg where SerialNum in ("&SerialNum&")")
			response.write "###"
		else
...
elseif showType="getInfo" then 
  ...
  elseif detailType="SerialNum" then
    InfoID=request("InfoID")
		sql="select * from Financesys_Suppliereg where SerialNum="&InfoID
		set rs=server.createobject("adodb.recordset")
		rs.open sql,connzxpt,1,1
		if rs.bof and rs.eof then
...
elseif showType="Export" then 
		sql="select * from Financesys_Suppliereg where Startdate>='"&request("SDate")&"' and Startdate<='"&request("EDate")&"' order by Startdate"
		set rs=server.createobject("adodb.recordset")
		rs.open sql,connzxpt,1,1
		%>
    <table border="1">
