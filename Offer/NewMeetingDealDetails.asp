...
<%
if Instr(session("AdminPurviewFLW"),"|304,")=0 then 
...
elseif showType="DataProcess" then 
  ...
  if detailType="AddNew" then
  	if  Instr(session("AdminPurviewFLW"),"|304.1,")>0 then
		set rs = server.createobject("adodb.recordset")
		sql="select * from Offer_NewMeetingDeal"
		...
		for i=0 to rs.fields.count-1
		  ...
		  elseif rs.fields(i).name ="MeetingDate" or rs.fields(i).name ="SolveDate" then
		  if Request("MeetingDate")<>"" then rs("MeetingDate")=Request("MeetingDate")
		  if Request("SolveDate")<>"" then rs("SolveDate")=Request("SolveDate")
		  else
  ...
  elseif detailType="Edit"  then
	...
	sql="select * from Offer_NewMeetingDeal where SerialNum="&SerialNum
	rs.open sql,connzxpt,1,3
  ...
  elseif detailType="Delete" then
	...
	rs.close
	set rs=nothing 
	connzxpt.Execute("Delete from Offer_NewMeetingDeal where SerialNum="&SerialNum)
	response.write "###"
  end if
elseif showType="getInfo" then 
  ...
  elseif detailType="SerialNum" then
    InfoID=request("InfoID")
	sql="select * from Offer_NewMeetingDeal where SerialNum="&InfoID
	set rs=server.createobject("adodb.recordset")
	rs.open sql,connzxpt,1,1
    if rs.bof and rs.eof then
