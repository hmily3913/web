...
if showType="DetailsList" then 
  ...
  dim datafrom'数据表名
      datafrom=" Offer_Honor a,Offer_HonorType b "
  dim datawhere'数据条件
	...
	End if
	if Request.Form("KeyWork") <> "" then datawhere=datawhere&" and (KeyWord like '%"&Request.Form("KeyWork")&"%' or HonorName like '%"&Request.Form("KeyWork")&"%') "
  dim sqlid'本页需要用到的id
...
%>		
		{"id":"<%=rs("SerialNum")%>",
		"cell":["<%=rs("SerialNum")%>","<%=rs("TypeID")%>","<%=rs("TypeName")%>","<%=Jsonstr(rs("HonorName"))%>","<%=Jsonstr(rs("Years"))%>","<%=rs("Biller")%>","<%=rs("BillDate")%>"]}
<%		
	    rs.movenext
		If Not rs.eof Then
		  Response.Write ","
...
elseif showType="DataProcess" then 
  ...
  if detailType="AddNew" then
  	if  Instr(session("AdminPurviewFLW"),"|307.1,")=0 then
			response.write ("你没有权限进行此操作！")
			response.end
		end if
		set rs = server.createobject("adodb.recordset")
		sql="select * from Offer_HonorType"
		...
		response.write "###"
		set rs = server.createobject("adodb.recordset")
		sql="select top 1 * from Offer_HonorType order by serialnum desc"
		rs.open sql,connzxpt,1,1
  ...
  elseif detailType="Edit"  then
    ...
    SerialNum=request("SerialNum")
		sql="select * from Offer_HonorType where SerialNum="&Request("TypeID")
		rs.open sql,connzxpt,1,3
...
'		end if
  ...
  elseif detailType="Delete" then
		...
		set rs = server.createobject("adodb.recordset")
    SerialNum=request("SerialNum")
		sql="select * from Offer_HonorType where SerialNum="&SerialNum
		rs.open sql,connzxpt,1,1
		if rs.bof and rs.eof then
			response.write ("数据库读取记录出错！")
			response.end
		...
		rs.close
		set rs=nothing 
		connzxpt.Execute("Delete from Offer_HonorType where SerialNum="&SerialNum)
		response.write "###"
  ...
  elseif detailType="EditFile" then
  	if  Instr(session("AdminPurviewFLW"),"|307.1,")=0 then
		...
		set rs = server.createobject("adodb.recordset")
		sql="select * from Offer_Honor where SerialNum="&request("SerialNum")
		rs.open sql,connzxpt,1,3

		if rs("BillerID")<>UserName then
			response.Write("只能编辑自己申请的荣誉！")
			response.End()
		end if
		rs("HonorName")=Request("HonorName")
		rs("KeyWord")=Request("KeyWords")
  ...
  elseif detailType="DeleteFile" then
  	if  Instr(session("AdminPurviewFLW"),"|307.1,")=0 then
		...
		rs.open sql,connzxpt,1,1
		if rs("BillerID")<>UserName then
			response.Write("只能删除自己添加的荣誉！")
			response.End()
		end if
		connzxpt.Execute("Delete from Offer_Honor where SerialNum="&request("SerialNum"))
		response.write "删除成功！"
  end if
elseif showType="getInfo" then 
  ...
  if detailType="SerialNum" then
    InfoID=request("InfoID")
		sql="select a.*,b.TypeName from Offer_Honor a,Offer_HonorType b where a.TypeID=b.SerialNum and  a.SerialNum="&InfoID
		set rs=server.createobject("adodb.recordset")
		rs.open sql,connzxpt,1,1
			if rs.bof and rs.eof then
    ...
    set rs = server.createobject("adodb.recordset")
    sql="select * from Offer_HonorType order by SerialNum"
    rs.open sql,connzxpt,1,1
		response.Write("[")
		do until rs.eof
	%>
	...
	<%
		...
		sql2="select count(1) as idcount from Offer_HonorType where PSNum="&rs("SerialNum")
	...
	elseif detailType="getModel" then
		sql="select HonorName from Offer_Honor "
		set rs=server.createobject("adodb.recordset")
		rs.open sql,connzxpt,1,1
		response.write "["
		do until rs.eof
		Response.Write("{""HonorName"":"""&JsonStr(rs("HonorName"))&"""}")
	    rs.movenext
		If Not rs.eof Then
		  Response.Write ","
		End If
    loop
		...
		rs.close
		set rs=nothing 
	elseif detailType="showHonor" then
		sql="select * from Offer_Honor where SerialNum="&request("InfoID")
		set rs = server.createobject("adodb.recordset")
		rs.open sql,connzxpt,1,3
		if rs.eof and rs.bof then
			response.Write("对应荣誉已经不存在，请刷新再查看！")
			response.End()
		else
