...
if showType="DetailsList" then 
  ...
  dim datawhere'数据条件
	...
	End if
	if Request.Form("KeyWork") <> "" then datawhere=datawhere&" and KnifePlate like '%"&Request.Form("KeyWork")&"%' "
...
%>		
		{"id":"<%=rs("SerialNum")%>",
		"cell":["<%=rs("SerialNum")%>","<%=rs("PositionID")%>","<%=rs("OrderId")%>","<%=rs("KnifePlate")%>","<%=rs("Biller")%>","<%=rs("BillDate")%>"]}
<%		
	    rs.movenext
		If Not rs.eof Then
		  Response.Write ","
...
elseif showType="DataProcess" then 
  ...
  if detailType="AddNew" then
  	if  Instr(session("AdminPurviewFLW"),"|306.1,")=0 then
			response.write ("你没有权限进行此操作！")
			response.end
		end if
		set rs = server.createobject("adodb.recordset")
		sql="select * from Offer_ShelfLayer"
		...
		response.write "###"
		set rs = server.createobject("adodb.recordset")
		sql="select top 1 * from Offer_ShelfLayer order by serialnum desc"
		rs.open sql,connzxpt,1,1
  ...
  elseif detailType="Edit"  then
    ...
    SerialNum=request("SerialNum")
		sql="select * from Offer_ShelfLayer where SerialNum="&Request("PositionID")
		rs.open sql,connzxpt,1,3
...
'		end if
  ...
  elseif detailType="Delete" then
		...
		set rs = server.createobject("adodb.recordset")
    SerialNum=request("SerialNum")
		sql="select * from Offer_ShelfLayer where SerialNum="&SerialNum
		rs.open sql,connzxpt,1,1
		if rs.bof and rs.eof then
			response.write ("数据库读取记录出错！")
			response.end
		...
		rs.close
		set rs=nothing 
		connzxpt.Execute("Delete from Offer_ShelfLayer where SerialNum="&SerialNum)
		response.write "###"
  elseif detailType="AddFile" then
  	if  Instr(session("AdminPurviewFLW"),"|306.1,")=0 then
		...
		set rs = server.createobject("adodb.recordset")
		sql="select * from Offer_KnifePlate"
		rs.open sql,connzxpt,1,3
		rs.addnew
		rs("KnifePlate")=Request("KnifePlate")
  ...
  elseif detailType="EditFile" then
  	if  Instr(session("AdminPurviewFLW"),"|306.1,")=0 then
		...
		set rs = server.createobject("adodb.recordset")
		sql="select * from Offer_KnifePlate where SerialNum="&request("DocumentID")
		rs.open sql,connzxpt,1,3
		rs("KnifePlate")=Request("KnifePlate")
		rs("OrderId")=Request("OrderId")
		rs("PositionID")=Request("PositionIDDetail")
  ...
  elseif detailType="DeleteFile" then
  	if  Instr(session("AdminPurviewFLW"),"|306.1,")=0 then
		...
		connzxpt.Execute("Delete from Offer_KnifePlate where SerialNum="&request("SerialNum"))
		response.write "删除成功！"
  end if
elseif showType="getInfo" then 
  ...
  if detailType="SerialNum" then
    InfoID=request("InfoID")
		sql="select * from Offer_KnifePlate where SerialNum="&InfoID
		set rs=server.createobject("adodb.recordset")
		rs.open sql,connzxpt,1,1
			if rs.bof and rs.eof then
    ...
    set rs = server.createobject("adodb.recordset")
    sql="select * from Offer_ShelfLayer order by SerialNum"
    rs.open sql,connzxpt,1,1
		response.Write("[")
		do until rs.eof
	%>
	...
	<%
		...
		sql2="select count(1) as idcount from Offer_ShelfLayer where PSNum="&rs("SerialNum")
	...
	elseif detailType="getModel" then
		sql="select KnifePlate from Offer_KnifePlate "
		set rs=server.createobject("adodb.recordset")
		rs.open sql,connzxpt,1,1
		response.write "["
		do until rs.eof
		Response.Write("{""KnifePlate"":"""&JsonStr(rs("KnifePlate"))&"""}")
	    rs.movenext
		If Not rs.eof Then
		  Response.Write ","
		End If
