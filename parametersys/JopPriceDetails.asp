...
if showType="DetailsList" then 
  ...
  dim datafrom'数据表名
      datafrom=" parametersys_JobPrice "
  dim datawhere'数据条件
  dim i'用于循环的整数
    datawhere=" where 1=1 "
	dim whereever
...
%>		
		{"id":"<%=rs("SerialNum")%>",
		"cell":["<%=rs("SerialNum")%>","<%=rs("Position")%>","<%=rs("JobPrice")%>"]}
<%		
	    rs.movenext
		If Not rs.eof Then
		  Response.Write ","
...
elseif showType="DataProcess" then 
  ...
  if detailType="AddNew" then
  	if  Instr(session("AdminPurviewFLW"),"|406.1,")>0 then
		set rs = server.createobject("adodb.recordset")
		sql="select * from parametersys_JobPrice"
  ...
  elseif detailType="Edit"  then
	...
	sql="select * from parametersys_JobPrice where SerialNum="&SerialNum
	rs.open sql,connzxpt,1,3
  ...
  elseif detailType="Delete" then
		...
		rs.close
		set rs=nothing 
		connzxpt.Execute("Delete from parametersys_JobPrice where SerialNum="&SerialNum)
		response.write "###"
  end if
elseif showType="getInfo" then 
  ...
  if detailType="SerialNum" then
	...
	sql="select * from parametersys_JobPrice where SerialNum="&InfoID
	set rs=server.createobject("adodb.recordset")
	rs.open sql,connzxpt,1,1
    if rs.bof and rs.eof then
