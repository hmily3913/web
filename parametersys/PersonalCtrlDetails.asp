...
<%
if Instr(session("AdminPurviewFLW"),"|402,")=0 then 
...
if showType="DetailsList" then 
  ...
  dim datafrom'数据表名
      datafrom=" parametersys_PersonalCtrl "
  dim datawhere'数据条件
  dim i'用于循环的整数
    datawhere=" where 1=1 "
	dim whereever
...
%>		
		{"id":"<%=rs("SerialNum")%>",
		"cell":["<%=rs("SerialNum")%>","<%=rs("MainDepart")%>","<%=rs("SubDepart")%>","<%=rs("PersonalType")%>","<%=rs("NeedNum")%>"]}
<%		
	    rs.movenext
		If Not rs.eof Then
		  Response.Write ","
...
elseif showType="DataProcess" then 
  ...
  if detailType="AddNew" then
  	if  Instr(session("AdminPurviewFLW"),"|402.1,")>0 then
		set rs = server.createobject("adodb.recordset")
		sql="select * from parametersys_PersonalCtrl"
		rs.open sql,connzxpt,1,3
		rs.addnew
		
		for i=0 to rs.fields.count-1
		  if rs.fields(i).name <>"SerialNum" then
  ...
  elseif detailType="Edit"  then
	...
	sql="select * from parametersys_PersonalCtrl where SerialNum="&SerialNum
	rs.open sql,connzxpt,1,3
  ...
  elseif detailType="Delete" then
	...
	rs.close
	set rs=nothing 
	connzxpt.Execute("Delete from parametersys_PersonalCtrl where SerialNum="&SerialNum)
	response.write "###"
...
elseif showType="getInfo" then 
  ...
  if detailType="SerialNum" then
	...
	sql="select * from parametersys_PersonalCtrl where SerialNum="&InfoID
	set rs=server.createobject("adodb.recordset")
	rs.open sql,connzxpt,1,1
    if rs.bof and rs.eof then
