...
"and a.fcombobox1 = '网络检修' and a.FBillStatus='维修'  "&_
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(当日有效数) as idCount1,sum(当日总数) as idCount2,个人 from managesys_NetworkrepairSum where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"' group by 个人"
	...
	Response.Write "[<font color='red'>"&rs("个人")&"</font>]统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，网络维修笔数为0<br>"
	else
    Reachper=Reachsum/unReachsum*100
    Response.Write "[<font color='red'>"&rs("个人")&"</font>]统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，网络维修笔数[<font color='red'>"&unReachsum&"</font>]，网络维修及时笔数[<font color='red'>"&Reachsum&"</font>]，网络维修及时率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]<br>"
	end if
	rs.movenext
	wend
  else
	sqlstr2="select * from managesys_NetworkrepairSum where datediff(d,UPTdate,getdate())=1"
    set rs=server.createobject("adodb.recordset")
    ...
    Response.Write "[<font color='red'>"&rs("个人")&"</font>]统计时间：[<font color='red'>昨日</font>]，网络维修笔数[<font color='red'>"&rs("当日总数")&"</font>]，网络维修及时笔数[<font color='red'>"&rs("当日有效数")&"</font>]，网络维修及时率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]<br>"
    Response.Write "　　　　统计时间：[<font color='red'>本月</font>]，网络维修笔数[<font color='red'>"&rs("本月总数")&"</font>]，网络维修及时笔数[<font color='red'>"&rs("本月有效数")&"</font>]，网络维修及时率为：[<font color='red'>"&formatnumber(Reachsum,2)&"%</font>]<br>"
	rs.movenext
	wend
  end if
  rs.close
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>网络维修及时率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="Networkrepair.asp?Result=Search">
          <td nowrap> 产品检索：从
          <script language=javascript> 
		...
		<a href="Networkrepair.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("网络维修延误")'>网络维修延误</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
 ...
 <%
  ...
  dim datawhere'数据条件
      if Keyword="list" then
	     datawhere="where CheckFlag>1 and RepairType='网络维修' and isnull(datediff(hh,ReplyFinishDate,RReplyDate),1)>6 and datediff(d,ReplyFinishDate,'" & StartDate & "')<=0 and datediff(d,ReplyFinishDate,'" & EndDate & "')>=0"
	  else
		 datawhere="where CheckFlag>1 and RepairType='网络维修' and isnull(datediff(hh,ReplyFinishDate,RReplyDate),1)>6 and datediff(d,ReplyFinishDate,getdate())=1"
 	  end if
  dim sqlid'本页需要用到的id
