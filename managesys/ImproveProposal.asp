...
<%
if Instr(session("AdminPurview"),"|1005,")=0 then 
...
sqlstr="select t_Improve_proposalEntry.fentryid,t_emp.fname as 提案人,Department1.fname as 提案部门, "&_
"FComboBox as 提案类别,t_Improve_proposalEntry.FNOTE as 改善前做法,FNOTE1 as 改善后做法, "&_
"FDecimal as 节约金额,FDecimal as 奖金,FDate2 as 提案日期  "&_
"from t_Improve_proposalEntry inner join t_Improve_proposal  "&_
"on t_Improve_proposalEntry.fid=t_Improve_proposal.fid LEFT OUTER JOIN "&_
"dbo.t_Department AS Department1 ON Department1.FItemID = t_Improve_proposalEntry.Fbase  "&_
"LEFT OUTER JOIN dbo.t_Base_Emp AS t_emp ON t_emp.FItemID = t_Improve_proposalEntry.Fbase1  "&_
"where fuser>0 and FDate2>='"&StartDate&"' and FDate2<='"&EndDate&"'"
...
function PlaceFlag()
  if Result="Search" then
    dim rs,sql'sql语句
  sql="select count(fentryid) as idCount from ("&sqlstr&") as aaa " 
  set rs=server.createobject("adodb.recordset")
 
  rs.open sql,connk3,0,1
  Reachsum=rs("idCount")
    if Reachsum=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，累计提案的次数为0"
	else
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，累计提案的次数为[<font color='red'>"&Reachsum&"</font>]"
	end if
  else
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>改善提案信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="ImproveProposal.asp?Result=Search">
          <td nowrap> 产品检索：从
...
<a href="ImproveProposal.asp?Result=Search&Keyword=all&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("查全部批次")'>全部批次</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>提案日期</strong></font></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>提案人</strong></font></td>
    <td width="60" height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>提案部门</strong></font></td>
    <td width="120" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>提案类别</strong></font></td>
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>改善前做法</strong></font></td>
    <td width="120" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>改善后做法</strong></font></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">节约金额</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">奖金</font></strong></td>
  </tr>
 <%
  dim idCount'记录总数
  dim pages'每页条数
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    sql="select * from ("& datafrom &") as aaa where fentryid in("& sqlid &") "&taxis
    set rs=server.createobject("adodb.recordset")
    rs.open sql,connk3,0,1
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("提案日期")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("提案人")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("提案部门")&"</td>"
      Response.Write "<td nowrap>"&rs("提案类别")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("改善前做法")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("改善后做法")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("节约金额")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("奖金")&"</td>" & vbCrLf
