...
<%
if Instr(session("AdminPurview"),"|504,")=0 then 
...
sqlstr="  SELECT     TOP (100) PERCENT v1.FDate AS 日期,v_ICTransType.Fname,  v1.FBillNo AS 单据编号, u1.FEntryID AS 行号,  "&_
"       ICItem.FNumber AS 物料代码, ICItem.FName AS 物料名称, Unit1.FName AS 单位, u1.FAuxQtyMust AS 帐存数量,  "&_
"       u1.FAuxQty AS 损益数量, u1.FAuxQtyActual as 实存数量, "&_
"       u1.FBatchNo AS 批号,  u1.FMTONo AS 计划跟踪号, StockPlace.FName AS 仓位,  "&_
"       Stock.FName AS 仓库, Emp2.FName AS 仓库员,  "&_
"       v1.FCheckDate AS 审核日期, USER1.FName AS 制单,  "&_
"       emp1.FName AS 验收, USER2.FName AS 审核, "&_
"       (LTRIM(RTRIM(STR(u1.FInterID)))+ 't' +LTRIM(RTRIM(STR(u1.FEntryID)))) AS id "&_
"FROM         dbo.ICStockBillEntry AS u1 INNER JOIN "&_
"       dbo.ICStockBill AS v1 ON u1.FInterID = v1.FInterID AND (v1.FTranType = 40 or v1.FTranType = 43) LEFT OUTER JOIN "&_
"       dbo.t_ICItemCore AS ICItem ON u1.FItemID = ICItem.FItemID LEFT OUTER JOIN "&_
"       dbo.t_ICItemBase AS t1 ON ICItem.FItemID = t1.FItemID LEFT OUTER JOIN "&_
...
"       dbo.t_Base_Emp AS emp1 ON v1.FFManagerID = emp1.FItemID LEFT OUTER JOIN "&_
"       dbo.t_ICItemPlan AS t3 ON ICItem.FItemID = t3.FItemID LEFT OUTER JOIN "&_
"       dbo.t_Emp AS Emp2 ON t3.FPlanner = Emp2.FItemID LEFT OUTER JOIN "&_
"       dbo.t_Stock AS Stock ON u1.FDCStockID = Stock.FItemID LEFT OUTER JOIN "&_
"       dbo.t_StockPlace AS StockPlace ON u1.FDCSPID = StockPlace.FSPID left outer join  "&_
"       v_ICTransType on v1.FTranType=v_ICTransType.finterid "&_
"where v1.FDate>='"&StartDate&"' and v1.FDate<='"&EndDate&"' and v1.FCheckerID > 0  "


function PlaceFlag()
  dim rs,sql,sqlstr2'sql语句
  if Result="Search" then
	sqlstr2="select sum(CardAROne) as idCount1,sum(unCardAROne) as idCount2 from stocksys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    if unReachsum=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，总盘点笔数为0"
	else
    Reachper=Reachsum/unReachsum*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，总盘点笔数[<font color='red'>"&unReachsum&"</font>]，帐卡物准确数[<font color='red'>"&Reachsum&"</font>]，帐卡物准确率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
	sqlstr2="select top 1 CardAROne as idCount1,unCardAROne as idCount2,CardAccuracyRateOne from stocksys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("CardAccuracyRateOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，总盘点笔数[<font color='red'>"&unReachsum&"</font>]，帐卡物准确数[<font color='red'>"&Reachsum&"</font>]，帐卡物准确率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>帐卡物准确率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="CardAccuracyRate.asp?Result=Search">
		...
		<a href="CardAccuracyRate.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("帐卡物不准列表")'>帐卡物不准列表</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>盘点类型</strong></font></td>
    <td height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>损益单号</strong></font></td>
    ...
    <td width="40" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>单位</strong></font></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">帐存数量</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">损益数量</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">实存数量</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">仓库</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">仓库员</font></strong></td>
 ...
 <%
  ...
  dim datafrom'数据表名
      datafrom="stocksys_CardAccuracyRate"
  dim datawhere'数据条件
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("单据编号")&"</td>"
      ...
      Response.Write "<td nowrap>"&rs("帐存数量")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("损益数量")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("实存数量")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("仓库")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("仓库员")&"</td>" & vbCrLf
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'CARreply',"&rs("SerialNum")&")"">"&rs("a20")&"</td>" & vbCrLf
      Response.Write "</tr>" & vbCrLf
	  rs.movenext
    wend
    Response.Write "<tr>" & vbCrLf
    Response.Write "<td colspan='10' nowrap  bgcolor='#EBF2F9'>&nbsp;</td>" & vbCrLf
