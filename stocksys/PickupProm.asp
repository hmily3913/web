...
<%
if Instr(session("AdminPurview"),"|506,")=0 then 
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(PickupPOne) as idCount1,sum(unPickupPOne) as idCount2 from stocksys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
	...
	sqlstr2="select top 1 PickupPOne as idCount1,unPickupPOne as idCount2,PickupPromOne from stocksys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("PickupPromOne")
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>提货及时率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="PickupProm.asp?Result=Search">
		...
		<a href="PickupProm.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("未及时提货列表")'>未及时提货列表</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'PPreply',"&rs("SerialNum")&")"">"&rs("a20")&"</td>" & vbCrLf
