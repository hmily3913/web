...
<%
if Instr(session("AdminPurview"),"|503,")=0 then 
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(DeliveryROne) as idCount1,sum(unDeliveryROne) as idCount2 from stocksys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
	...
	sqlstr2="select top 1 DeliveryROne as idCount1,unDeliveryROne as idCount2,FinishDeliveryRateOne from stocksys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("FinishDeliveryRateOne")
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>销售发货及时率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="FinishDeliveryRate.asp?Result=Search">
		...
		<a href="FinishDeliveryRate.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("超交期列表")'>超时发货列表</a>
		|<input name='output' type='button' class='button'  id='output' value='引出' onClick='window.open("FinishDeliveryRatePrint.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1","Print","","false")'>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">回复人</font></strong></td>
  </tr>
 <%
  dim idCount'记录总数
  dim pages'每页条数
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'FDRreply',"&rs("SerialNum")&")"">"&rs("a20")&"</td>" & vbCrLf
