...
<%
if Instr(session("AdminPurview"),"|507,")=0 then 
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(SendCarPOne) as idCount1,sum(unSendCarPOne) as idCount2 from stocksys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    if unReachsum=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，总派车数为0"
	else
    Reachper=Reachsum/unReachsum*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，总派车笔数[<font color='red'>"&unReachsum&"</font>]，及时派车次数[<font color='red'>"&Reachsum&"</font>]，派车及时率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
	sqlstr2="select top 1 SendCarPOne as idCount1,unSendCarPOne as idCount2,SendCarPromOne from stocksys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("SendCarPromOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，总派车笔数[<font color='red'>"&unReachsum&"</font>]，及时派车次数[<font color='red'>"&Reachsum&"</font>]，派车及时率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>派车及时率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="SendCarProm.asp?Result=Search">
		...
		<a href="SendCarProm.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("未及时派车列表")'>未及时派车列表</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>派车原因</strong></font></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>货物名称</strong></font></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">送货地点</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">计划出发时间</font></strong></td>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("GoodsName")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("DeliveryAddr")&"</td>" & vbCrLf
      ...
      Response.Write "<td nowrap>"&rs("CheckFlag")&"</td>" & vbCrLf
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'SCPreply',"&rs("SerialNum")&")"">"&rs("a20")&"</td>" & vbCrLf
