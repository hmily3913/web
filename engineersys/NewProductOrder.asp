...
<%
if Instr(session("AdminPurview"),"|704,")=0 then 
...
sqlstr=" SELECT     dbo.[I-型录内外资料表 ].型号,  dbo.[I-型录内外资料表 ].新旧产品, dbo.[I-型录内外资料表 ].名称, "&_                                   
"  dbo.[I-型录内外资料表 ].生产部门, dbo.[I-型录内外资料表 ].日期, dbo.[I-型录内外资料表 ].确认, dbo.[I-型录内外资料表 ].业务员, "&_              
"  dbo.[I-型录内外资料表 ].设计者, "&_                                                                                                            
"  dbo.[I-型录内外资料表 ].科长, dbo.[I-型录内外资料表 ].文员, dbo.[I-型录内外资料表 ].制作者, dbo.[I-型录内外资料表 ].制图者, "&_                
"  dbo.[I-型录内外资料表 ].设计者比例, dbo.[I-型录内外资料表 ].科长比例, dbo.[I-型录内外资料表 ].文员比例, dbo.[I-型录内外资料表 ].制作者比例, "&_
"  dbo.[I-型录内外资料表 ].制图者比例, dbo.[I-型录内外资料表 ].团队比例, dbo.[OR-订单接单明细统计表报表 ].客户代号, "&_                           
"  dbo.[OR-订单接单明细统计表报表 ].订单单号,  dbo.[OR-订单接单明细统计表报表 ].序号, "&_                                                         
"  dbo.[OR-订单接单明细统计表报表 ].订单日期, dbo.[OR-订单接单明细统计表报表 ].品号, dbo.[OR-订单接单明细统计表报表 ].品名规格, "&_               
"  dbo.[OR-订单接单明细统计表报表 ].部门名称,  dbo.[OR-订单接单明细统计表报表 ].订单数量, "&_                                                     
"  dbo.[OR-订单接单明细统计表报表 ].业务员姓名, dbo.[OR-订单接单明细统计表报表 ].换算后金额, "&_                                                  
"  dbo.[OR-订单接单明细统计表报表 ].计划跟踪号 ,[OR-订单接单明细统计表报表].订单单号+LTRIM(RTRIM(STR([OR-订单接单明细统计表报表].序号))) as id"&_                                                                                                
" FROM   dbo.[OR-订单接单明细统计表报表 ] INNER JOIN "&_                                                                                          
"  dbo.[I-型录内外资料表 ] ON dbo.[OR-订单接单明细统计表报表 ].型号 = dbo.[I-型录内外资料表 ].型号 "&_                                            
" where  dbo.[I-型录内外资料表 ].新旧产品='自主开发' and dbo.[I-型录内外资料表 ].确认=1 and "&_                                                   
" dbo.[OR-订单接单明细统计表报表 ].订单日期>='"&StartDate&"' and dbo.[OR-订单接单明细统计表报表 ].订单日期<='"&EndDate&"' "
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(NewProductOrderOne) as idCount2 from engineersys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    unReachsum=rs("idCount2")
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，新产品接单额[<font color='red'>"&unReachsum&"</font>]"
  else
	sqlstr2="select top 1 NewProductOrderOne as idCount2 from engineersys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    unReachsum=rs("idCount2")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，新产品接单额[<font color='red'>"&unReachsum&"</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>新产品接单额</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="NewProductOrder.asp?Result=Search">
		...
		<a href="NewProductOrder.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("查全部批次")'>全部批次</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">订单日期</font></strong></td>
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>订单号</strong></font></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">品号</font></strong></td>
    <td  nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>型号</strong></font></td>
    <td  height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>名称</strong></font></td>
    <td width="80" height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>设计者</strong></font></td>
    <td  nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>制作者</strong></font></td>
    ...
    <td width="80" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>换算后金额</strong></font></td>
 ...
 <%
  ...
  dim datafrom'数据表名
      datafrom="engineersys_NewProductOrder"
  dim datawhere'数据条件
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("订单单号")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("品号")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("型号")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("名称")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("设计者")&"</td>"
      Response.Write "<td nowrap>"&rs("制作者")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("业务员")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("订单数量")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("换算后金额")&"</td>" & vbCrLf
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'NPOreply',"&rs("SerialNum")&")"">"&rs("a20")&"</td>" & vbCrLf
