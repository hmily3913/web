...
<%
if Instr(session("AdminPurview"),"|703,")=0 then 
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(NewProductDevOne) as idCount2 from engineersys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    unReachsum=rs("idCount2")
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，新产品开发件数[<font color='red'>"&unReachsum&"</font>]"
  else
	sqlstr2="select top 1 NewProductDevOne as idCount2 from engineersys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    unReachsum=rs("idCount2")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，新产品开发件数[<font color='red'>"&unReachsum&"</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>新产品开发件数</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="NewProductDev.asp?Result=Search">
		...
		<a href="NewProductDev.asp?Result=Search&Keyword=all&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("查全部批次")'>全部批次</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td  nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>编号</strong></font></td>
    <td  height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>产品型号</strong></font></td>
    <td width="80" height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>产品规格</strong></font></td>
    ...
    <td nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">设计</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">制作</font></strong></td>
    <td  nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">单价</font></strong></td>
 ...
 <%
  ...
  dim datafrom'数据表名
      datafrom="engineersys_NewProductDev"
  dim datawhere'数据条件
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("产品规格")&"</td>"
      Response.Write "<td nowrap>"&rs("日期")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("产品名称")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("设计")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("制作")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("单价")&"</td>" & vbCrLf
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'NPDreply',"&rs("SerialNum")&")"">"&rs("a20")&"</td>" & vbCrLf
