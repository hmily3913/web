...
<%
if Instr(session("AdminPurview"),"|701,")=0 then 
...
sqlstr="SELECT 订单日期,审核日期,打样单号,序号,客户代号,业务员姓名,品号,名称,数量,开发回复交期,业务交期,打样员,结案,确认,逾期码,结案日期,  "&_
"(LTRIM(RTRIM(打样单号))+ 't' +LTRIM(RTRIM(序号))) AS id "&_
"FROM [K-打样单查询] "&_
"where 确认=1 and 订单日期 >='"&StartDate&"' and 订单日期 <='"&EndDate&"' "
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(ProofingFinishOne) as idCount1,sum(unProofingFinishOne) as idCount2 from engineersys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    if unReachsum=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，打样笔数为0"
	else
    Reachper=Reachsum/unReachsum*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，打样笔数[<font color='red'>"&unReachsum&"</font>]，打样按期完成笔数[<font color='red'>"&Reachsum&"</font>]，打样按期完成率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
	sqlstr2="select top 1 ProofingFinishOne as idCount1,unProofingFinishOne as idCount2,ProofingFinishRateOne from engineersys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("ProofingFinishRateOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，打样笔数[<font color='red'>"&unReachsum&"</font>]，打样按期完成笔数[<font color='red'>"&Reachsum&"</font>]，打样按期完成率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>打样按期完成率</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="ProofingFinishRate.asp?Result=Search">
		...
		<a href="ProofingFinishRate.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("打样未按期列表")'>打样未按期列表</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td width="80" height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>打样单号</strong></font></td>
    <td width="80" height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>客户代号</strong></font></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>业务员</strong></font></td>
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>产品代号</strong></font></td>
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>产品名称</strong></font></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">数量</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">回复交期</font></strong></td>
    <td width="30" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>业务交期</strong></font></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">结案日期</font></strong></td>
 ...
 <%
  ...
  dim datafrom'数据表名
      datafrom="engineersys_ProofingFinishRate"
  dim datawhere'数据条件
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("订单日期")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("打样单号")&"</td>"
      Response.Write "<td nowrap>"&rs("客户代号")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("业务员姓名")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("品号")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("名称")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("数量")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("开发回复交期")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("业务交期")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("结案日期")&"</td>" & vbCrLf
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'PFRreply',"&rs("SerialNum")&")"">"&rs("a20")&"</td>" & vbCrLf
