...
<%
if Instr(session("AdminPurview"),"|705,")=0 then 
...
sqlstr=" select a.fid,a.fbillno,a.FBillNo_SRC,a.FBase,a.ftext,t_emp.fname, "&_   
"a.fdate,a.fdate1,a.fdate2,a.ftext6,datediff(d,a.fdate1,a.fdate2) as diffdays "&_   
"from t_Antenatal_try as a left join t_emp on  "&_   
"t_emp.fitemid=a.ftext5 "&_   
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(TryQualifOne) as idCount1,sum(unTryQualifOne) as idCount2 from engineersys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    if unReachsum=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，产前试做笔数为0"
	else
    Reachper=Reachsum/unReachsum*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，产前试做笔数[<font color='red'>"&unReachsum&"</font>]，产前试做按期完成笔数[<font color='red'>"&Reachsum&"</font>]，产前试做完成率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
	sqlstr2="select top 1 TryQualifOne as idCount1,unTryQualifOne as idCount2,TryQualifiedOne from engineersys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("TryQualifiedOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，产前试做笔数[<font color='red'>"&unReachsum&"</font>]，产前试做按期完成笔数[<font color='red'>"&Reachsum&"</font>]，产前试做完成率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>产前试做合格率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="TryQualified.asp?Result=Search">
		...
		<a href="TryQualified.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("未完成列表")'>未完成列表</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td  height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>订单号</strong></font></td>
    ...
    <td nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">试做日期</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">计划完成日期</font></strong></td>
    <td  nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">实际完成日期</font></strong></td>
    <td width="80" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>试做要求</strong></font></td>
 ...
 <%
  ...
  dim datafrom'数据表名
      datafrom="engineersys_TryQualified"
  dim datawhere'数据条件
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("FBillNo_SRC")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("FBase")&"</td>"
      ...
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'TQreply',"&rs("SerialNum")&")"">"&rs("a20")&"</td>" & vbCrLf
