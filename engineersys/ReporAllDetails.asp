...
elseif showType="getChart" then 
  sql="select yearnum as 年份,Period as 月份,sum(ProofingFinishRate) as 打样按期完成率,sum(NewProductDev) as 新产品开发件数, "&_
		"sum(NewProductOrder) as 新产品接单额,sum(TryQualified) as 产前试做合格率, "&_
		"sum(BomFinish) as BOM表按期完成率 "&_
		"from ( "&_
		"select yearnum,Period,ProofingFinishRate,NewProductDev, "&_
		"NewProductOrder,TryQualified,0 as BomFinish   "&_
		"from engineersys a, "&_
		"(select max(uptdate) as 月末日期 from engineersys  "&_
		...
		"union all "&_
		"select yearnum,Period-1 as Period,0 as ProofingFinishRate,0 as NewProductDev, "&_
		"0 as NewProductOrder,0 as TryQualified,BomFinish   "&_
		"from engineersys a, "&_
		"(select min(uptdate) as 月初日期 from engineersys  "&_
		"where yearnum=datepart(yy,getdate()) and uptdate is not null  "&_
