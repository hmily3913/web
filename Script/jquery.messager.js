 ...
 */
    ...
    this.show = function(title, text, time){
        ...
        $("#message_min").click(function(){
        	if($("#message_min").text()=='□'){
            setTimeout('this.max()', 1);
        	}
        	else{
            setTimeout('this.min()', 1);
          }
        });
        //$("#message").slideDown('slow');
        this.minmessage(this.time);
        if(this.autohide)this.rmmessage(this.time);
    };
    ...
    this.minmessage = function(time){
        setTimeout('this.min()', time);
    };
    this.close = function(){
        switch(this.anims.type){
    ...
    this.min = function(){
	  if($("#message").is("div")){
		$("#message_min").text('□');
        $("#message").animate({height:25,opacity: 0.3});
				$("#message").mouseenter(function(e){$("#message").animate({opacity: 1});});
				$("#message").mouseleave(function(e){$("#message").animate({opacity: 0.3});});
        this.original();
	  }
    };
    this.max = function(){
	  if($("#message").is("div")){
		$("#message_min").text('_');
        $("#message").animate({height:this.layer.height,opacity: 1});
				$("#message").unbind();
        this.original();
	  }
    };
