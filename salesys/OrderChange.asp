...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(OrderChangesOne) as idCount1,sum(OrdersOne) as idCount2 from Salesys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    ...
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，共有订单数[<font color='red'>"&unReachsum&"</font>]，变更笔数[<font color='red'>"&Reachsum&"</font>]，订单更改率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
	sqlstr2="select top 1 OrderChangesOne as idCount1,OrdersOne as idCount2,OrderChangeOne from Salesys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("OrderChangeOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，共有订单数[<font color='red'>"&unReachsum&"</font>]，变更笔数[<font color='red'>"&Reachsum&"</font>]，订单更改率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>订单更改率信息</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="OrderChange.asp?Result=Search">
		...
		<a href="OrderChange.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("订单变更列表")'>订单变更列表</a><font color="#0000FF"></a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>变更日期</strong></font></td>
    <td width="80" height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>订单号</strong></font></td>
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>变更单号</strong></font></td>
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>业务员</strong></font></td>
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>变更类型</strong></font></td>
    ...
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">变更数量</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">变更金额</font></strong></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">变更原因</font></strong></td>
 ...
 <%
  ...
  dim taxis'排序的语句 asc,desc
      taxis=" order by id,fentryid asc"
  dim i'用于循环的整数
  dim rs,sql'sql语句
  '获取记录总数
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("变更日期")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("销售订单号")&"</td>"
      ...
      Response.Write "<td nowrap>"&rs("变更类型")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("fnumber")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("fname")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("falterqty")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("falteramount")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("变更原因")&"</td>" & vbCrLf
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'OCreply',"&rs("fentryid")&")"">"&rs("a1")&"</td>" & vbCrLf
