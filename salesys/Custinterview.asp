...
<BODY>
<%
dim Result,StartDate,EndDate,Keyword,Reachsum1,Reachsum2,Reachper,sqlstr
...
sqlstr=" select t_Cust_interview.FBillNo as id, Organization.fname as 客户,Department1.fname as 部门,emp.fname as 业务员,t_Cust_interview.FDate,t_Cust_interview.FComboBox,  "&_           
" t_Cust_interview.FText,t_Cust_interview.FText1,t_Cust_interview.FNOTE,t_Cust_interview.FNOTE1 from t_Cust_interview INNER JOIN "&_
" dbo.t_Organization AS Organization ON Organization.FItemID = t_Cust_interview.FBase  LEFT OUTER JOIN  "&_   
" dbo.t_Department AS Department1 ON Department1.FItemID = t_Cust_interview.FBase1  LEFT OUTER JOIN  "&_             
"dbo.t_Emp AS emp ON t_Cust_interview.FBase2 = emp.FItemID where Fuser>0 and FDate>='"&StartDate&"' and FDate<='"&EndDate&"'  "         
...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(CustinterviewOne) as idCount1 from Salesys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，拜访/接待客户记录[<font color='red'>"&Reachsum&"</font>]次"
  else
	sqlstr2="select top 1 CustinterviewOne as idCount1 from Salesys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，拜访/接待客户记录[<font color='red'>"&Reachsum&"</font>]数"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>客户访谈记录</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="Custinterview.asp?Result=Search">
          <td nowrap> 信息检索：从<input type="text" class="textfield" style="width:80px" id="start_date" name="start_date" value="<%=StartDate%>"/>
		...
		<a href="Custinterview.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("客户访谈记录列表")'>客户访谈记录列表</a><font color="#0000FF"></a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>客户</strong></font></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>访谈日期</strong></font></td>
	<td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>编号</strong></font></td>
	<td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>部门</strong></font></td>
    <td width="80" height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>业务员</strong></font></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>类别</strong></font></td>
    <td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>客户人员</strong></font></td>
    <td width="120" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">主题</font></strong></td>
    <td width="260" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">访谈内容</font></strong></td>
    <td width="260" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">客户建议</font></strong></td>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
	  ...
	  dim temword2
	  temword2=rs("id")
	  if temword2<>temword then 
      Response.Write "<td nowrap>"&rs("客户")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("FDate")&"</td>"
      Response.Write "<td nowrap>"&temword2&"</td>" & vbCrLf
	  temword=temword2
	  else
	  ...
	  Response.Write "<td nowrap>"&rs("FNOTE1")&"</td>" & vbCrLf
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'CVreply',"&rs("id")&")"">"&rs("a1")&"</td>" & vbCrLf
...
'-----------------------------------------------------------
  ...
  Response.Write "<tr>" & vbCrLf
  Response.Write "<td>共计："&idcount&"</font>条记录&nbsp;页次：<font color='#ff6600'>"&page&"</font></strong>/"&pagec&"&nbsp;每页：<font color='#ff6600'>"&pages&"</font>条</td>" & vbCrLf
