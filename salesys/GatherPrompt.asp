...
function PlaceFlag()
  ...
  if Result="Search" then
	sqlstr2="select sum(GatherOne) as idCount1 from Salesys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，实收金额[<font color='red'>"&Reachsum&"</font>]"
  else
	sqlstr2="select top 1 GatherOne as idCount1,unGatherOne as idCount2,GatherPromptOne from Salesys order by SerialNum desc"
	set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    Reachper=rs("GatherPromptOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，实收金额[<font color='red'>"&Reachsum&"</font>]，应收金额[<font color='red'>"&unReachsum&"</font>]，应收款完成率为：[<font color='red'>"&Reachper&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    ...
    <td width="60" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">财务日期</font></strong></td>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'GPreply','"&rs("id")&"')"">"&rs("a1")&"</td>" & vbCrLf
