...
elseif showType="getChart" then 
  sql="select yearnum as 年份,Period as 月份,sum(OrderChange) as 订单更改率,sum(OrderDeliverRate) as 订单出货达成率, "&_
		"sum(GatherPrompt) as 收款完成率,sum(Custinterview) as 拜访接待客户 "&_
		"from salesys a, "&_
		"(select max(uptdate) as 月末日期 from salesys  "&_
