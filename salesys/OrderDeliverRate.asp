...
sqlstr="SELECT     TOP (100) PERCENT Organization.FName AS 客户, dbo.t_BOS200000007.FName AS 单据类别, "&_  
"                      SEOrder.FBillNo AS 销售订单号, ICItem.FNumber AS 产品代码, ICItem.FName AS 产品名称, SEOrderEntry.fdate as 交货日期, "&_
"                      Department2.FName AS 生产车间,  SEOrderEntry.FQty AS 订单数量,   SEOrderEntry.FStockQty AS 出货数量,  emp.FName AS 业务员,  "&_
"                      SEOrderEntry.FMTONo AS 计划跟踪号, User3.FName AS 生管制单人, SEOrder.FDate AS 制单日期,max(ICStockBill.fdate) 最后出货日期, "&_
"					 (LTRIM(RTRIM(STR(SEOrderEntry.FInterID)))+ 't' +LTRIM(RTRIM(STR(SEOrderEntry.FEntryID)))) AS id, "&_
"					 case when max(ICStockBill.fdate) is null then DATEDIFF ( d  , SEOrderEntry.fdate , getdate()) "&_
"					 else DATEDIFF ( d  , SEOrderEntry.fdate , max(ICStockBill.fdate)) end as diffdays "&_
"FROM                 dbo.SEOrderEntry AS SEOrderEntry  INNER JOIN "&_
...
"                      dbo.t_Department AS Department2 ON Department2.FItemID = SEOrderEntry.FEntrySelfS0177 LEFT OUTER JOIN "&_
"                      dbo.t_ICItem AS ICItem ON SEOrderEntry.FItemID = ICItem.FItemID left outer join  "&_
"					  dbo.ICStockBillEntry AS ICStockBillEntry on ICStockBillEntry.FOrderInterID=SEOrderEntry.FInterID and ICStockBillEntry.FOrderEntryID=SEOrderEntry.FEntryID LEFT OUTER JOIN "&_
"                      dbo.ICStockBill AS ICStockBill ON ICStockBillEntry.FBrNo = ICStockBill.FBrNo AND ICStockBillEntry.FInterID = ICStockBill.FInterID and ICStockBill.FTranType = 21 "&_
"WHERE     ISNULL(SEOrder.FCancellation, 0) = 0 and SEOrder.fstatus>0 "&_
"group by Organization.FName , dbo.t_BOS200000007.FName ,   "&_
"                      SEOrder.FBillNo , ICItem.FNumber, ICItem.FName, SEOrderEntry.fdate , "&_
"                      Department2.FName,  SEOrderEntry.FQty,   SEOrderEntry.FStockQty,  emp.FName,  "&_
"                      SEOrderEntry.FMTONo, User3.FName, SEOrder.FDate,SEOrderEntry.FInterID,SEOrderEntry.FEntryID "&_
"having SEOrderEntry.fdate>='"&StartDate&"' and SEOrderEntry.fdate<='"&EndDate&"'"


function PlaceFlag()
  dim rs,sql,sqlstr2'sql语句
  if Result="Search" then
	sqlstr2="select sum(unDeliversOne) as idCount1,sum(DeliversOne) as idCount2 from Salesys where  UPTdate>='"&StartDate&"' and UPTdate<='"&EndDate&"'"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
    if unReachsum=0 then
	Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，共有订单数为0"
	else
    Reachper=Reachsum/unReachsum*100
    Response.Write "统计时间：[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，共有订单数[<font color='red'>"&unReachsum&"</font>]，达成笔数[<font color='red'>"&Reachsum&"</font>]，订单发货达成率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
	end if
  else
	sqlstr2="select top 1 unDeliversOne as idCount1,DeliversOne as idCount2,OrderDeliverRateOne from Salesys order by SerialNum desc"
    set rs=server.createobject("adodb.recordset")
    rs.open sqlstr2,connzxpt,0,1
    Reachsum=rs("idCount1")
    unReachsum=rs("idCount2")
	Reachper=rs("OrderDeliverRateOne")
    Response.Write "统计时间：[<font color='red'>昨日</font>]，共有订单数[<font color='red'>"&unReachsum&"</font>]，达成笔数[<font color='red'>"&Reachsum&"</font>]，订单发货达成率为：[<font color='red'>"&formatnumber(Reachper,2)&"%</font>]"
  end if
  rs.close
  set rs=nothing
end function  
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td height="24" nowrap class="tablemenu"><font color="#FFFFFF"><img src="../Images/images/flexigrid/grid.png" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>订单出货达成率</strong></font></td>
  </tr>
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="OrderDeliverRate.asp?Result=Search">
		...
		<a href="OrderDeliverRate.asp?Result=Search&Keyword=list&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("超交期列表")'>超时收货列表</a><font color="#0000FF">&nbsp;|&nbsp;</font><a href="OrderDeliverRate.asp?Result=Search&Keyword=all&start_date=<%=StartDate%>&end_date=<%=EndDate%>&Page=1" onClick='changeAdminFlag("查全部批次")'>全部批次</a>
		</td>
		<%end if%>
      </tr>
    </table>      </td>    
...
function ProductsList()
 ...
 <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#99BBE8">
  <tr>
    <td width="60" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>订单日期</strong></font></td>
    <td width="80" height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>订单号</strong></font></td>
    <td width="80" height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>客户名称</strong></font></td>
    <td width="60" height="24" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>业务员</strong></font></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>产品代码</strong></font></td>
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>产品名称</strong></font></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">订单数量</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">出货数量</font></strong></td>
    <td width="30" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>交货日期</strong></font></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">出货日期</font></strong></td>
    <td width="50" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">工作回复</font></strong></td>
  </tr>
 <%
  dim idCount'记录总数
  dim pages'每页条数
  ...
  dim datafrom'数据表名
      datafrom="sale_OrderDeliverRate"
  dim datawhere'数据条件
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    sql="select *,left(ReplyText,10) as a1 from "& datafrom &" where id in("& sqlid &") "&taxis
    set rs=server.createobject("adodb.recordset")
    rs.open sql,connzxpt,0,1
	dim tempstr,bgcolors
	tempstr=""
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap>"&rs("客户")&"</td>"
      Response.Write "<td nowrap>"&rs("业务员")&"</td>"
	  tempstr=tempstr2
	  else 
      ...
      Response.Write "<td nowrap>"&rs("出货数量")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("交货日期")&"</td>" & vbCrLf
      Response.Write "<td nowrap>"&rs("最后出货日期")&"</td>" & vbCrLf
      Response.Write "<td nowrap onDblClick=""return SAClickTd(this,'ODRreply','"&rs("id")&"')"">"&rs("a1")&"</td>" & vbCrLf
